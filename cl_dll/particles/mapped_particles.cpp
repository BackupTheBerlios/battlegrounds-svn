...
CMappedParticleSystem::CMappedParticleSystem( char *sParticleDefinition, particle_system_management *pSysDetails ) : CParticleSystem()
{
	...
	if(LoadParticleDefinition()) {
		...
		pParticleTexture = LoadTGA(pParticleTexture, m_pSystem->sParticleTexture);

		// add all the starting particles we were asked to
		int iParticles = StartingParticles();
		for(int i = 0; i <iParticles; i++) {
			CParticle *pParticle = new CMappedParticle(m_pSystem, pParticleTexture);
...
//		return;
	...
	// these are finite particles, so create particles at a exponentially falling rate
		...
		if((flLongestLife / m_flSystemMaxAge) < 0.9999f) {
			while((m_flParticleCreationTime > 0) && (m_flSystemsAge < (0.9999f * m_flSystemsAge))) {
				CParticle *pParticle = new CMappedParticle(m_pSystem, pParticleTexture);
				AddParticle(pParticle);
				...
				if((m_pSystem->iGenerationFalloff == LINEAR_HALF_LIFETIME) &&  (flLongestLife > (m_flSystemMaxAge * 0.5))) {
					...
					m_flParticleCreationTime -= 1.0f / ( flParticleDelay * (1 - (m_flSystemsAge / m_flSystemMaxAge)));
				}
			}
		}
	}
...
void CMappedParticleSystem::CreateDefaultParticleSystem( void )
{
	...
	m_pSystem->bPlanar = false;
	m_pSystem->bSmoke = false;
	m_pSystem->bSnowRain = false;
}
...
bool CMappedParticleSystem::LoadParticleDefinition( void )
{
	...
	if(!sFile) {
		gEngfuncs.Con_Printf("Bad Mapped Particle definition file specified");
		return false;
	}
	...
	while(bLooping) {
		...
		// how long the plane is if SHAPE_PLANE has been set
		...
		// the system faces out
		else if(!stricmp(sSetting, "fade_out")) {
			m_pSystem->bFadeOut = (!!(atoi(sValue)));
		}
		// planar ??
		else if(!stricmp(sSetting, "planar")) {
			m_pSystem->bPlanar = (!!(atoi(sValue)));
		}
		// smoke ??
		...
		// smoke ??
		else if(!stricmp(sSetting, "snow_rain")) {
			m_pSystem->bSnowRain = (!!(atoi(sValue)));
		}
