...
protected:
	...
	inline void AddParticle( CParticle *pParticle );
public:
	// draw all associated particles
	virtual bool DrawSystem();
	// returns the id of a ps
	inline unsigned int SystemID( void ) const { return m_iID; }
...
protected:
	// returns the number of particles to be created on creation of this system
	inline unsigned int StartingParticles( grass_particle_types *pGrassType );
