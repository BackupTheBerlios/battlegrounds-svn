...
CGrassParticleSystem::CGrassParticleSystem( char *sParticleDefinition, particle_system_management *pSysDetails) : CParticleSystem()
{
	...
	if(LoadParticleDefinition(pSysDetails)) {
		...
		for(unsigned int i = 0; i < m_cGrassTypes.size(); i++) {
			...
			pGrassType->pParticleTexture = LoadTGA(pGrassType->pParticleTexture, pGrassType->pSystem->sParticleTexture);

			if(pGrassType->pParticleTexture == NULL)
				continue;

			// add all the starting particles we were asked to
			int iGrassPercentage = (int)(CVAR_GET_FLOAT("cl_grassamount")/100);

			if (iGrassPercentage < 0)
				iGrassPercentage = 0;
			if (iGrassPercentage > 50)
				iGrassPercentage = 50;

			int iParticles = StartingParticles(pGrassType) * iGrassPercentage;
...
// returns the number of particles that we are to create for this grass type when init'ing the system
unsigned int CGrassParticleSystem::StartingParticles( grass_particle_types *pGrassType )
{
	unsigned int iParticles =  static_cast<unsigned int>(g_ParticleCount->value);
	return (pGrassType->pSystem->iCount * iParticles / PERCENT);		
}
...
bool CGrassParticleSystem::LoadParticleDefinition( particle_system_management *pSysDetails )
{
	...
	if(!sFile) {
		gEngfuncs.Con_Printf("Bad Grass Particle definition file specified");
		return false;
	}
