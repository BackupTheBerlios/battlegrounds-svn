	...
	for ( i = 0; i < maxHUDMessages; i++ )
	{
		if ( !m_pMessages[i] )
		{
			...
			if ( !tempMessage )
			{
				...
				_snprintf(g_pCustomText, sizeof(g_pCustomText)-1, "%s\0", pName);
				g_pCustomMessage.pMessage = g_pCustomText;

				tempMessage = &g_pCustomMessage;
			}
			_snprintf(g_pCustomText, sizeof(g_pCustomText)-1, "\n%s\n\0", tempMessage->pMessage);
			tempMessage->pMessage = g_pCustomText;
...
int CHudMessage::MsgFunc_HudText( const char *pszName,  int iSize, void *pbuf )
{
	...
	char *pString = READ_STRING(sizeof(g_pCustomText));
