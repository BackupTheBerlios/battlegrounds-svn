...
class CBlackPanel;//add new panel type non tranpsarent Myristate
class CBritClassMenu;
//Ben - For the american classes
class CAmerClassMenu;
//Ben
...
//Ben
/*
extern char *sBGClassSelection[];
extern char *sBGClassSelection2[];
extern char *sBGLocalisedClasses[];
extern char *sBGLocalisedClasses2[];
*/
//Ben
...
public:
	SpectButton( const char* text,int x,int y,int wide,int tall ) : 
...
private:
	...
	void		 CreateBritClassMenu( void );
	CMenuPanel*	 ShowBritClassMenu( void );
	//Ben - Creates & shows our second class menu
	void		 CreateAmerClassMenu( void );
	CMenuPanel*	 ShowAmerClassMenu( void );
	//Ben
	...
	int			m_iValidClasses[NUM_CLASSES];
	int			m_iNumberOfTeams;
	char		m_sTeamNames[NUM_TEAMS][MAX_TEAMNAME_SIZE];
...
public:
	...
	// Data Handlers
	int GetValidClasses( void) { return NUM_CLASSES; };
	int GetNumberOfTeams( void ) { return NUM_TEAMS; };
	char *GetTeamName( int iTeam ) { return (char*)szTeamNames[iTeam]; };
	int GetAllowSpectators( void ) { return m_iAllowSpectators; };
	...
	int MsgFunc_ResetScores( const char *pszName, int iSize, void *pbuf );
	int MsgFunc_FullScore( const char *pszName, int iSize, void *pbuf );
	//BP ParticleEmitter
	...
	int MsgFunc_HintBox( const char *pszName, int iSize, void *pbuf );
	//BP Commandermenu
//	int MsgFunc_ResComPoint( const char *pszName, int iSize, void *pbuf );
...
public:
	...
	CBritClassMenu	*m_pBritClassMenu;
	//Ben - Pointer to our 2nd class menu
	CAmerClassMenu *m_pAmerClassMenu;
	//Ben
...
public:
	CMenuHandler_StringCommand( char *pszCommand, bool bClose = false )
	{
		strncpy( m_pszCommand, pszCommand, MAX_COMMAND_SIZE);
		m_pszCommand[MAX_COMMAND_SIZE-1] = '\0';
		m_iCloseVGUIMenu = bClose;
	}
...
private:
public:
	CMenuHandler_StringCommandWatch( char *pszCommand, bool bClose = false ) : CMenuHandler_StringCommand( pszCommand, bClose )
	{
	}
	...
	virtual void actionPerformed(Panel* panel)
	{
		...
		if ( !strcmp( m_pszCommand, "jointeam 1" ) ) {
			g_iTeamNumber = 1;
			gViewPort->ShowVGUIMenu( MENU_BRITISH_CLASS );
		}
		else if ( !strcmp( m_pszCommand, "jointeam 2" ) ) {
			g_iTeamNumber = 2;
			gViewPort->ShowVGUIMenu( MENU_AMERICAN_CLASS );
		}
		...
		{
			char temp[64];
			sprintf(temp, "joinclass %i", i+1);

			if(!strcmp( m_pszCommand, temp))
				g_iPlayerClass = i;
		}
	}
};
...
public:
	...
	CMenuHandler_StringCommandClassSelect( char *pszCommand, bool bClose ) : CMenuHandler_StringCommand( pszCommand, bClose )
	{
	}
...
//================================================================
// Ok a little explenaion its a bit liel transparent panel but i didnt
// like the transparency and wanted complete darkness so i made this lol 
//simply just a chnage of transparency on the alpha channel
class CBlackPanel : public Panel
{
private:
	int	m_iTransparency;
public:
	CBlackPanel(int iTrans, int x,int y,int wide,int tall) : Panel(x,y,wide,tall)
	{
		m_iTransparency = iTrans;
	}
...
public:
	...
	int			ShouldBeRemoved( void ) { return m_iRemoveMe; };
	CMenuPanel* GetNextMenu( void ) { return m_pNextMenu; };
	int			GetMenuID( void ) { return m_iMenuID; };
	int			IsActive( void ) { return m_iIsActive; };
	float		GetOpenTime( void ) { return m_flOpenTime; };
...
//============================================================
class CBritClassMenu : public CMenuPanel
{
public:
	CTransparentPanel	*m_pPanel;
	CommandButton		*m_pOfficer;
	CommandButton		*m_pSoldier;
	CommandButton		*m_pLoyalist;
	CommandButton		*m_pAutoAssign;
	CommandButton		*m_pCancel;
	CImageLabel			*m_pBackground;
	CImageLabel			*m_pClasses[NUM_CLASSES + 1];

public:
	CBritClassMenu(int iTrans, int iRemoveMe, int x,int y,int wide,int tall);
...
//Ben - For the american class menu
class CAmerClassMenu : public CMenuPanel
{
public:
	CTransparentPanel	*m_pPanel;
	CommandButton		*m_pOfficer;
	CommandButton		*m_pSoldier;
	CommandButton		*m_pMinuteMan;
	CommandButton		*m_pAutoAssign;
	CommandButton		*m_pCancel;
	CImageLabel			*m_pBackground;
	CImageLabel			*m_pClasses[NUM_CLASSES + 1];
...
public:
	CAmerClassMenu(int iTrans, int iRemoveMe, int x,int y,int wide,int tall);
...
{
private:
	CTransparentPanel	*m_pClassInfoPanel[NUM_CLASSES+1];
	Label				*m_pPlayers[NUM_CLASSES+1];
	ClassButton			*m_pButtons[NUM_CLASSES+1];
	CommandButton		*m_pCancelButton;
	ScrollPanel			*m_pScrollPanel;

//	CImageLabel			*m_pClassImages[NUM_TEAMS][NUM_CLASSES];
	CImageLabel			*m_pClassImages[NUM_CLASSES+1];
...
public:
	CTransparentPanel	*m_pPanel;
	int					m_iShowText;
	CommandButton		*m_pBritish;
	CommandButton		*m_pAmerican;
	CommandButton		*m_pSpectatorEye;
	CommandButton		*m_pBritish2;
	CommandButton		*m_pAmerican2;
	CommandButton		*m_pSpectator2;
	CommandButton		*m_pAutoAssign;
	CommandButton		*m_pCancel;
	CImageLabel			*m_pBackground;
...
public:
	...
	void paint()
	{
		...
		if (gHUD.m_Health.m_flHealth <= 5.0)
			a = 255;
		...
		int x = gHUD.DrawHudNumber( iXPos + iXSize + 5, iYPos + 5, DHN_3DIGITS | DHN_DRAWZERO, gHUD.m_Health.m_flHealth, r, g, b);
