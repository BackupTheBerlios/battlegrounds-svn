...
team_info_t			 g_TeamInfo[NUM_TEAMS+2]; // brits, amers, specs, connecting
...
{
	{NULL,			4,			Label::a_west},
	{NULL,			160,		Label::a_west},		// name
	{"#CLASS",		85,			Label::a_west},		// class
	{"#DEAD",		30,			Label::a_west},		//death staus
	{"#CAPTURES",	70,			Label::a_east},	
	{"#DAMAGE",		86,			Label::a_east},
	{"#LATENCY",	25,			Label::a_east},
	{"#VOICE",		54,			Label::a_east},
...
ScorePanel::ScorePanel(int x,int y,int wide,int tall) : Panel(x,y,wide,tall)
{
	...
	LineBorder *border = new LineBorder(Color(255, 255, 255, 0));
	setBorder(border);
...
void ScorePanel::Initialize( void )
{
	...
	g_TeamInfo[BRITS].capture_points = 0;
	g_TeamInfo[AMERS].capture_points = 0;
}
...
void ScorePanel::Update()
{
	...
	if (gViewPort->m_szServerName)
	{
		...
		sprintf(sz, "BG: %s", gViewPort->m_szServerName );
...
void ScorePanel::SortTeams()
{
	...
	{
		g_TeamInfo[i].capture_points += g_TeamInfo[i].extra_points;
	...
	for ( i = 1; i < MAX_PLAYERS; i++ )
	{
		...
		if ( j > NUM_TEAMS )  // player is not in a team, skip to the next guy
			continue;

		g_TeamInfo[j].damage_done += g_PlayerExtraInfo[i].damage_done;
	...
	while ( bLooping )
	{
		long int highest_captures = -999999; long int highest_damage = -999999;
		int best_team = 0;
		...
		for ( i = 1; i <= NUM_TEAMS; i++ )
		{
			...
			if ( !g_TeamInfo[i].already_drawn && g_TeamInfo[i].capture_points >= highest_captures )
			{
				if ( g_TeamInfo[i].capture_points > highest_captures || g_TeamInfo[i].damage_done > highest_damage )
				{
					best_team = i;
					highest_captures = g_TeamInfo[i].capture_points;
					highest_damage = g_TeamInfo[i].damage_done;
				}
			}
		}
...
void ScorePanel::SortPlayers( int iTeam, char *team )
{
	...
	while ( bLooping )
	{
		...
		long int highest_points = -999999;	long int highest_damage = -999999;
		int best_player;
		best_player = 0;

		for ( int i = 1; i < MAX_PLAYERS; i++ )
		{
			if ( m_bHasBeenSorted[i] == false && g_PlayerInfoList[i].name && g_PlayerExtraInfo[i].capture_points >= highest_points )
			{
				...
				if ( ent && !(team && stricmp(g_PlayerExtraInfo[i].teamname, team)) )  
				{
					...
					if ( pl_info->capture_points> highest_points || pl_info->damage_done > highest_damage )
					{
						best_player = i;
						highest_points = pl_info->capture_points;
						highest_damage = pl_info->damage_done;
					}
				}
			}
		}
...
void ScorePanel::RebuildTeams()
{
	...
	sprintf( g_TeamInfo[BRITS].name, "%s\0", British );
	sprintf( g_TeamInfo[AMERS].name, "%s\0", Americans );
	sprintf( g_TeamInfo[SPECS].name, "%s\0", Spectators );

	g_TeamInfo[BRITS].players = 0;
	g_TeamInfo[AMERS].players = 0;
	g_TeamInfo[SPECS].players = 0;
	...
	for ( i = 1; i < MAX_PLAYERS; i++ )
	{
		...
		if ( g_PlayerInfoList[i].spectator == 1) {
			_snprintf(g_PlayerExtraInfo[i].teamname, sizeof(g_PlayerExtraInfo[i].teamname) - 1, "%s\0", Spectators);
			g_PlayerExtraInfo[i].teamnumber = SPECS;
		}
...
void ScorePanel::FillGrid()
{
	...
	for(int row=0; row < NUM_ROWS; row++)
	{
		...
		for(int col=0; col < NUM_COLUMNS; col++)
		{
			...
			{
				// team color text for team names
				pLabel->setFgColor(	iTeamColors[4 % iNumberOfTeamColors][0],
									iTeamColors[4 % iNumberOfTeamColors][1],
									iTeamColors[4 % iNumberOfTeamColors][2],
									0 );
				...
				pGridRow->SetRowUnderline(	0,
											true,
											YRES(3),
											iTeamColors[4 % iNumberOfTeamColors][0],
											iTeamColors[4 % iNumberOfTeamColors][1],
											iTeamColors[4 % iNumberOfTeamColors][2],
											0 );
			}
			else
			{
				...
				else
				{
					...
					if ( pl_info->thisplayer ) // if it is their name, draw it a different color
					{
						// Highlight this player
						pLabel->setFgColor(Scheme::sc_white);
						pLabel->setBgColor(	iTeamColors[ 4 % iNumberOfTeamColors ][0],
											iTeamColors[ 4 % iNumberOfTeamColors ][1],
											iTeamColors[ 4 % iNumberOfTeamColors ][2],
											196 );
					}
			...
			// Align 
			if (col == COLUMN_NAME || col == COLUMN_CLASS || col == COLUMN_DEAD)
			{
			...
			if ( m_iIsATeam[row] )
			{
				...
				case COLUMN_NAME:
					...
					{
						sprintf( sz2, CHudTextMessage::BufferedLocaliseTextString( "Connecting" ) );
					}
					else
					{
				...
				case COLUMN_CAPTURES:
					if ( m_iIsATeam[row] == TEAM_YES )
						sprintf(sz, "%d",  team_info->capture_points );
					break;

				case COLUMN_DAMAGE:
					if ( m_iIsATeam[row] == TEAM_YES )
						sprintf(sz, "%d",  team_info->damage_done );
					break;
				case COLUMN_LATENCY:
					if ( m_iIsATeam[row] == TEAM_YES )
			...
			else
			{
				...
				case COLUMN_CLASS:
					...
					if ( gViewPort && EV_TFC_IsAllyTeam(  g_PlayerExtraInfo[ m_iSortedRows[row] ].teamnumber ) || g_iTeamNumber == SPECS )
						bShowClass = true;
					...
					if (bShowClass)
					{
						...
						{
							if ( gViewPort->GetValidClasses( ) != -1 )
								bNoClass = true;
						}

						if (bNoClass)
							sprintf(sz, "");
						else if(g_PlayerExtraInfo[ m_iSortedRows[row] ].teamnumber == SPECS)
							sprintf(sz, "");
						else
							sprintf( sz, "%s", CHudTextMessage::BufferedLocaliseTextString( szClassNames[ g_PlayerExtraInfo[ m_iSortedRows[row] ].teamnumber ][ g_PlayerExtraInfo[ m_iSortedRows[row] ].playerclass ] ) );
					}
					else
					{
						strcpy(sz, "");
					...
					break;
				case COLUMN_DEAD:
					if ( g_PlayerExtraInfo[ m_iSortedRows[row] ].teamnumber == SPECS )
						sprintf(sz, "Spec");
					else if((g_PlayerExtraInfo[m_iSortedRows[row]].dead == true) && (EV_TFC_IsAllyTeam( g_PlayerExtraInfo[ m_iSortedRows[row] ].teamnumber) || (g_PlayerExtraInfo[ m_iSortedRows[row] ].teamnumber == UNDEFINED)))
						sprintf(sz, "Dead");
					else
						sprintf(sz, "");
					break;
				case COLUMN_TRACKER:
					break;
				case COLUMN_CAPTURES:
					sprintf(sz, "%d",  g_PlayerExtraInfo[ m_iSortedRows[row] ].capture_points );
					break;
				case COLUMN_DAMAGE:
					if(pl_info->thisplayer)
						sprintf(sz, "%d",  g_PlayerExtraInfo[ m_iSortedRows[row] ].damage_done );
					else
						sprintf(sz, " ");
					break;
				case COLUMN_LATENCY:
...
void ScorePanel::ExportTeamScores(int &iBritish, int &iAmericans)
{
	if(g_TeamInfo[BRITS].capture_points > 99999)
		g_TeamInfo[BRITS].capture_points = 0;
	iBritish = g_TeamInfo[BRITS].capture_points;
	if(g_TeamInfo[AMERS].capture_points > 99999)
		g_TeamInfo[AMERS].capture_points = 0;
	iAmericans = g_TeamInfo[AMERS].capture_points;
}
