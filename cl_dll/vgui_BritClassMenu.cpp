...
CBritClassMenu::CBritClassMenu(int iTrans, int iRemoveMe, int x,int y,int wide,int tall) : CMenuPanel(iTrans, iRemoveMe, x,y,wide,tall)
{
	cl_entity_t *player;
	...
	for (int i = 1; i <= NUM_CLASSES; i++)
	{
		...
		sprintf(sz, "%s", szClassNames[BRITS][i]);
	...
	//	}
		...
		sprintf(sz, "Class: %s", szClassNames[BRITS][i]);
		...
		if ( bShowClassGraphic )
		{
			sprintf( sz, "%s", szClassCodes[BRITS][i] );
		...
		sprintf(sz, "classes/%s.txt", szClassCodes[BRITS][i]);
...
/*
void CBritClassMenu::Reset(void)
{
    m_pCancelButton->setArmed( false );
    CMenuPanel::Reset();
}
*/
void CBritClassMenu::Initialize(void)
{
    setVisible(false);
}

// Update
void CBritClassMenu::Update()
{
...
bool CBritClassMenu::SlotInput( int iSlot )
{
...
void CBritClassMenu::SetActiveInfo( int iInput )
{
