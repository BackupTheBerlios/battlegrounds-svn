...
int iLastEventIdx = 0;
CWeaponData *g_WeaponData;

// Ben - Bg weapon class accessors
CKnife g_Knife;
CSabre g_Sabre;

CBrownBess g_BrownBess;
CCharleville g_Charleville;
CRevolutionnaire g_Revolutionnaire;
CPennsylvania g_Pennsylvania;
...
//Ben - Set the damage up
void CBaseEntity::SetWeaponDamage(float flDamage)
{
	player.m_pActiveItem->m_flSkillModifier = flDamage;
}
...
{
	PLAYBACK_EVENT_FULL(FEV_NOTHOST, m_pPlayer->edict(), pData->m_usAnimation, 0.0, m_pPlayer->pev->origin, m_pPlayer->pev->angles, 0.0, 0.0, iAnim, pev->body, 0, 0 );
}
...
void CBasePlayerWeapon::ItemPostFrame( void )
{
	...
	else if ( !(m_pPlayer->pev->button & (IN_ATTACK|IN_ATTACK2) ) )
	{
		...
		{
			gEngfuncs.Con_Printf("Client dll reload2\n");
			Reload();
			return;
		}
		*/
...
void HUD_InitClientWeapons( void )
{
	...
	// Ben - Prepares our client side weapons
	HUD_PrepEntity( &g_Knife, &player);
	HUD_PrepEntity( &g_Sabre, &player);

	HUD_PrepEntity( &g_BrownBess, &player);
	HUD_PrepEntity( &g_Revolutionnaire, &player);
	HUD_PrepEntity( &g_Charleville, &player);
	HUD_PrepEntity( &g_Pennsylvania, &player);

	HUD_PrepEntity( &g_PistolA, &player);
	HUD_PrepEntity( &g_PistolB, &player);
}
...
void HUD_WeaponsPostThink( local_state_s *from, local_state_s *to, usercmd_t *cmd, double time, unsigned int random_seed )
{
	...
	switch ( from->client.m_iId )
	{
		...
		case WEAPON_BESS:
			pWeapon = &g_BrownBess;
			g_WeaponData = g_BrownBess.pData;
			break;
		case WEAPON_CHARLE:
			pWeapon = &g_Charleville;
			g_WeaponData = g_Charleville.pData;
			break;
		case WEAPON_REVOL:
			pWeapon = &g_Revolutionnaire;
			g_WeaponData = g_Revolutionnaire.pData;
			break;
		case WEAPON_PENNY:
			pWeapon = &g_Pennsylvania;
			g_WeaponData = g_Pennsylvania.pData;
			break;

		case WEAPON_PISTOLA:
			pWeapon = &g_PistolA;
			g_WeaponData = g_PistolA.pData;
			break;
		case WEAPON_PISTOLB:
			pWeapon = &g_PistolB;
			g_WeaponData = g_PistolB.pData;
			break;
	}
	...
	for ( i = 0; i < 32; i++ )
	{
		...
		pCurrent->pev->body				= pfrom->fuser2;
		pCurrent->m_fNextStabTime		= pfrom->fuser3;
		pCurrent->m_chargeReady			= pfrom->iuser1;
		pCurrent->m_fInAttack			= pfrom->iuser2;
		pCurrent->m_fireState			= pfrom->iuser3;

		pCurrent->m_iSecondaryAmmoType		= (int)from->client.vuser2[ 0 ];
		pCurrent->m_iPrimaryAmmoType		= (int)from->client.vuser2[ 1 ];
		player.m_rgAmmo[ pCurrent->m_iPrimaryAmmoType ]	= (int)from->client.vuser2[ 2 ];
		player.m_rgAmmo[ pCurrent->m_iSecondaryAmmoType ]	= (int)from->client.vuser1[ 2 ];
	}
...
//	player.m_flAmmoStartCharge = from->client.fuser3;
	...
	player.ammo_musket_shots		= (int)from->client.vuser1[0];
...
//	to->client.fuser3					= player.m_flAmmoStartCharge;
	...
	//HL Weapons
	to->client.vuser1[0]				= player.ammo_musket_shots;
...
//		HUD_SendWeaponAnim( to->client.weaponanim, player.m_pActiveItem->pev->body, 1 );
	}
	...
	for ( i = 0; i < 32; i++ )
	{
		...
		pto->fuser3						= pCurrent->m_fNextStabTime;
		pto->fuser1						= pCurrent->pev->fuser1;
		pto->fuser2						= pCurrent->pev->body;
		...
		pto->fuser3						-= cmd->msec / 1000.0;

		to->client.vuser2[0]				= pCurrent->m_iSecondaryAmmoType;
		to->client.vuser2[1]				= pCurrent->m_iPrimaryAmmoType;
		to->client.vuser2[2]				= player.m_rgAmmo[ pCurrent->m_iPrimaryAmmoType ];
		to->client.vuser1[2]				= player.m_rgAmmo[ pCurrent->m_iSecondaryAmmoType ];
...
{
	// PAT: there is absolutely no other way to prevent 
	// clients from flipping this CVAR on, which avoids all kinds of
	// client prediction stuff (note: disabling the below check does nothing,
	// we had this bug in TOD, it can mess up things based on FOV and other
	// rare 'treats'.
...
CWeaponData* GetGunsData(struct event_args_s* args)
{
	CBasePlayerWeapon *pWeapon = NULL;
	CWeaponData *pData = NULL;
	...
	switch ( args->iparam2 )
	{
		...
		case WEAPON_BESS:
			pWeapon = &g_BrownBess;
			pData = g_BrownBess.pData;
			break;
		case WEAPON_CHARLE:
			pWeapon = &g_Charleville;
			pData = g_Charleville.pData;
			break;
		case WEAPON_REVOL:
			pWeapon = &g_Revolutionnaire;
			pData = g_Revolutionnaire.pData;
			break;
		case WEAPON_PENNY:
			pWeapon = &g_Pennsylvania;
			pData = g_Pennsylvania.pData;
			break;

		case WEAPON_PISTOLA:
			pWeapon = &g_PistolA;
			pData = g_PistolA.pData;
			break;
		case WEAPON_PISTOLB:
			pWeapon = &g_PistolB;
			pData = g_PistolB.pData;
			break;
		default:
			pData = NULL;
	}
	pData->iWeight = pWeapon->pev->body;
	return pData;
}