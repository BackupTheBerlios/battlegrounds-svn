...
const int WEAPON_SELECTION_TIME = 3;

WEAPON *gpActiveSel;	// NULL means off, 1 means just the menu bar, otherwise
						// this points to the active weapon menu item
WEAPON *gpLastSel;		// Last weapon menu selection 
float flWeaponDisplayTime = 0.0;
float flLastFrameTime = 0.0;
...
void WeaponsResource :: LoadWeaponSprites( WEAPON *pWeapon )
{
	...
	p = GetSpriteList( pList, "crosshair_bit", iRes, i );
	if (p)
	{
		sprintf(sz, "sprites/%s.spr", p->szSprite);
		pWeapon->hCrosshairBit = SPR_Load(sz);
		pWeapon->rcCrosshairBit = p->rc;
	}
	else
		pWeapon->hCrosshairBit = NULL;
...
void CHudAmmo::Think(void)
{
	...
	if (gHUD.m_iKeyBits & IN_ATTACK)
	{
		...
		{
			flWeaponDisplayTime = (flLastFrameTime + WEAPON_SELECTION_TIME);
...
{
	// Ben - Removed as this adds more restrictions in stationary reload mode than exist in normal play atm
	// Raiko Contrib - Stationary Reload	
//	extern bool inReload;

//	if((g_iStationaryReload == 1) && inReload)
//		return;
	// Raiko End
	...
	if ( (gpActiveSel == NULL) || (gpActiveSel == (WEAPON *)1) || (iSlot != gpActiveSel->iSlot) )
	{
		...
		if ( p ) // check for fast weapon switch mode
		{
...
int CHudAmmo::MsgFunc_WeaponList(const char *pszName, int iSize, void *pbuf )
{
	...
	strcpy( Weapon.szName, READ_STRING(MAX_WEAPON_NAME) );
...
int CHudAmmo::Draw(float flTime)
{
	...
	if ( (gHUD.m_iHideHUDDisplay & ( HIDEHUD_WEAPONS | HIDEHUD_HEALTH | HIDEHUD_ALL )) )
		return 1;
	...
	y = ScreenHeight - gHUD.m_iFontHeight;

	// Does weapon have any ammo at all?
	if (m_pWeapon->iAmmoType > 0)
	{
		if ((gWR.CountAmmo(pw->iAmmoType) < 4) && (gWR.CountAmmo(pw->iAmmoType) > 1))
			gHintBox.Set(HINTBOX_LOWAMMO);
		...
		int iAmmo = gWR.CountAmmo(pw->iAmmoType) + pw->iClip;
		x = gHUD.DrawHudNumber(x, y, DHN_3DIGITS, iAmmo, r, g, b, 3);
	}
	...
	if (pw->iAmmo2Type > 0) 
	{
		...
		if ((pw->iAmmo2Type != 0) && (gWR.CountAmmo(pw->iAmmo2Type) > 0))
		{
			...
			x = gHUD.DrawHudNumber(x, y, iFlags|DHN_3DIGITS, gWR.CountAmmo(pw->iAmmo2Type), r, g, b, 1);
...
int CHudAmmo::DrawWList(float flTime)
{
	...
	a = 128; //!!!
	x = 256 + 24; // hud base width + a bit more
	...
	for (i = 0; i < MAX_WEAPON_SLOTS; i++)
	{
		...
		if ( i == iActiveSlot )
		{
			...
			for ( int iPos = 0; iPos < MAX_WEAPON_POSITIONS; iPos++ )
			{
				...
				if ( gpActiveSel == p )
				{
					y = (YRES(480) - (p->rcActive.bottom - p->rcActive.top));
				...
				else
				{
					...
					y = (YRES(480) - (p->rcInactive.bottom - p->rcInactive.top));
