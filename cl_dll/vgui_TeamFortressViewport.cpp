...
void TeamFortressViewport::Initialize( void )
{
	...
	// 1 = scu_none ( VS 2005 express has a problem here )
	App::getInstance()->setCursorOveride( App::getInstance()->getScheme()->getCursor((vgui::Scheme::SchemeCursor)1) );
}
...
void TeamFortressViewport::UpdateCursorState()
{
	...
	if ( m_pSpectatorPanel->m_menuVisible || m_pCurrentMenu || m_pTeamMenu->isVisible() || m_pServerBrowser->isVisible() || GetClientVoiceMgr()->IsInSquelchMode() )
	{
		...
		// 2 = scu_arrow ( VS 2005 express has a problem here )
		App::getInstance()->setCursorOveride( App::getInstance()->getScheme()->getCursor((vgui::Scheme::SchemeCursor)2) );
		return;
	}
