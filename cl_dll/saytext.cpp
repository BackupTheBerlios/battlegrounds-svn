...
int CHudSayText :: MsgFunc_SayText( const char *pszName, int iSize, void *pbuf )
{
	...
	int iSizeOfMessage = 0;
	if(sizeof(pString) > iSize)
		iSizeOfMessage = sizeof(pString);
	else
		iSizeOfMessage = iSize;

	// string length is handled inside the function below
	SayTextPrint( pString, iSizeOfMessage - 1,  client_index );
...
void CHudSayText :: SayTextPrint( const char *pszBuf, int iBufSize, int clientIndex )
{
	...
	Y_START = ScreenHeight - g_Text_ypos->value;
