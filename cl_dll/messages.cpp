...
int CPredefinedMessage::MsgFunc_MakeMessage( const char *pszName, int iSize, void *pbuf)
{
	...
	else if(iMessageType == iCapturePointMessages)
	{
		...
		// multiple players required for capture
		...
		else if(iPointEvent == iPointEvent_Abandoned)
		{
			...
			// are we (client) involved in the capture?
			cl_entity_t *player = gEngfuncs.GetLocalPlayer();
			if ( iPlayerId == player->index) 
				gHUD.m_Points.ClearClock(iPointId);
		...
		else if(iPointEvent == iPointEvent_Player_Entered)
		{
			...
			if (gHUD.m_Points.m_aPoints[gHUD.m_Points.ReturnIndexById(iPointId)]->iStatus != iPointCaptured)
			{
				cl_entity_t *player = gEngfuncs.GetLocalPlayer();
				if ( iPlayerId == player->index)	
				{
		...
		// only 1 player required for capture
