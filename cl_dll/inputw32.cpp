...
extern int	cl_pitchdown;
extern int	cl_pitchup;
extern int	cl_yawspeed;
extern int	cl_sidespeed;
extern int	cl_forwardspeed;
extern int	cl_pitchspeed;
...
void IN_JoyMove ( float frametime, usercmd_t *cmd )
{
	...
	for (i = 0; i < JOY_MAX_AXES; i++)
	{
		...
		case AxisForward:
			if ((joy_advanced->value == 0.0) && (in_jlook.state & 1))
			{
				...
				{		
					...
					{
						viewangles[PITCH] -= (fAxisValue * joy_pitchsensitivity->value) * aspeed * cl_pitchspeed;
					}
					else
					{
						viewangles[PITCH] += (fAxisValue * joy_pitchsensitivity->value) * aspeed * cl_pitchspeed;
			...
			else
			{
				...
				{
					cmd->forwardmove += (fAxisValue * joy_forwardsensitivity->value) * speed * cl_forwardspeed;
				}
			}
			break;
		...
		case AxisSide:
			...
			{
				cmd->sidemove += (fAxisValue * joy_sidesensitivity->value) * speed * cl_sidespeed;
			}
			break;
		...
		case AxisTurn:
			...
			else
			{
				...
				if (fabs(fAxisValue) > joy_yawthreshold->value)
				{
					...
					{
						viewangles[YAW] += (fAxisValue * joy_yawsensitivity->value) * aspeed * cl_yawspeed;
					}
					else
					{
