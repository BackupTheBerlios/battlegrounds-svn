...
//kbutton_t	in_rush; //BP commanders menu
...
// BP commanders menu
//void IN_RushDown( void )
//{ 
//	KeyDown( &in_rush );
//	gHUD.m_CommanderMenu.PreExecute(iRush);
//};
//void IN_RushUp( void )
//{
//	KeyUp( &in_rush );
...
{
	gHintBox.Set(HINTBOX_JUMP);  // BPPPPPPPPPPPPPPPPPPPPPPPPPPP
	KeyDown(&in_jump);
...
{
	gHintBox.Set(HINTBOX_CROUCH);
	gHintBox.Set(HINTBOX_CROUCH2);
	KeyDown(&in_duck);
...
void CL_AdjustAngles ( float frametime, float *viewangles )
{
	...
	if (in_speed.state & 1)
	{
		speed = frametime * cl_anglespeedkey;
	}
	else
	{
		speed = frametime;
	...
	{
		viewangles[YAW] -= speed*cl_yawspeed*CL_KeyState (&in_right);
		viewangles[YAW] += speed*cl_yawspeed*CL_KeyState (&in_left);
		viewangles[YAW] = anglemod(viewangles[YAW]);
	}
	if (in_klook.state & 1)
	{
		V_StopPitchDrift ();
		viewangles[PITCH] -= speed*cl_pitchspeed * CL_KeyState (&in_forward);
		viewangles[PITCH] += speed*cl_pitchspeed * CL_KeyState (&in_back);
	}
	
	up = CL_KeyState (&in_lookup);
	down = CL_KeyState(&in_lookdown);
	
	viewangles[PITCH] -= speed*cl_pitchspeed * up;
	viewangles[PITCH] += speed*cl_pitchspeed * down;

	if (up || down)
		V_StopPitchDrift ();
		
	if (viewangles[PITCH] > cl_pitchdown)
		viewangles[PITCH] = cl_pitchdown;
	if (viewangles[PITCH] < -cl_pitchup)
		viewangles[PITCH] = -cl_pitchup;
...
//BP RushAura
	...
	if ( active )
	{
		cl_forwardspeed		= iClassSpeeds[g_iTeamNumber][g_iPlayerClass];
		cl_backspeed		= (iClassSpeeds[g_iTeamNumber][g_iPlayerClass] / 2);
		cl_sidespeed		= ((iClassSpeeds[g_iTeamNumber][g_iPlayerClass] / 4) * 3);
...
int	CL_IsDead( void )
{
	return ( gHUD.m_Health.m_flHealth <= 0.0 ) ? 1 : 0;
}
...
//	gEngfuncs.pfnAddCommand ("+rush",IN_RushUp);
//	gEngfuncs.pfnAddCommand ("-rush",IN_RushDown);
	...
	cl_upspeed			= 320;
	cl_forwardspeed		= iClassSpeeds[g_iTeamNumber][g_iPlayerClass];
	cl_backspeed		= (iClassSpeeds[g_iTeamNumber][g_iPlayerClass] / 3);
	cl_sidespeed		= (iClassSpeeds[g_iTeamNumber][g_iPlayerClass] / 2);
	cl_movespeedkey		= 0.3;
	cl_pitchup			= 89;
	cl_pitchdown		= 89;
