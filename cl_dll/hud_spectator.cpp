...
extern void V_GetDoubleTargetsCam(cl_entity_t * ent1, cl_entity_t * ent2,float * angle, float * origin);
...
void CHudSpectator::SetSpectatorStartPosition()
{
	...
	else if ( UTIL_FindEntityInMap( "info_player_spectator",  m_cameraOrigin, m_cameraAngles ) )
		iJumpSpectator = 1;

	else if ( UTIL_FindEntityInMap( "info_player_british",  m_cameraOrigin, m_cameraAngles ) )
		iJumpSpectator = 1;

	else if ( UTIL_FindEntityInMap( "info_player_american",  m_cameraOrigin, m_cameraAngles ) )
		iJumpSpectator = 1;
	else
	{
...
/*							if(g_iUser2 && g_iUser3)
							{
								cl_entity_t* pVictim;
								cl_entity_t* pKiller;
								pVictim = gEngfuncs.GetEntityByIndex(g_iUser2);
								pKiller = gEngfuncs.GetEntityByIndex(g_iUser3);

								if(IsActivePlayer(pVictim))
								{
									V_GetDoubleTargetsCam(pVictim, pKiller, pVictim->origin, pVictim->angles);
								}
							}
*/
		...
		case DRC_CMD_MESSAGE:
							{
								...
								strncpy( m_HUDMessageText[m_lastHudMessage], READ_STRING(128), 128 );
		...
		case DRC_CMD_SOUND :
							//no char buffer involved here that i can see
							string = READ_STRING();
							value =  READ_FLOAT();
		...
		case DRC_CMD_BANNER:
							...
							gViewPort->m_pSpectatorPanel->m_TopBanner->LoadImage( READ_STRING(256) );
							gViewPort->UpdateSpectatorPanel();
							break;
...
void CHudSpectator::SetModes(int iNewMainMode, int iNewInsetMode)
{
	...
	if ( iNewMainMode < OBS_NONE || iNewMainMode > OBS_MAP_CHASE )
	{
		gEngfuncs.Con_Printf("Invalid spectator mode.\n");
		return;
	}

	if( iNewMainMode == OBS_NONE )
	{
		g_iUser1 = 0;
		return;
	}
...
bool CHudSpectator::ParseOverviewFile( )
{
	...
	if (!pfile)
	{
		gEngfuncs.Con_Printf("Couldn't open file %s. Using default values for overview mode.\n", filename );
		return false;
	}
