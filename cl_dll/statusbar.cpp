...
int CHudStatusBar :: Draw( float fTime )
{
	...
	int Y_START = ((ScreenHeight / 8) * 5) - YRES(32);
	...
	for ( int i = 0; i < MAX_STATUSBAR_LINES; i++ )
	{
		...
		if ( (i == STATUSBAR_ID_LINE) )
		{
			...
			y = ((ScreenHeight / 8) * 5) + (TextHeight * CVAR_GET_FLOAT("hud_centerid"));
		}
...
int CHudStatusBar :: MsgFunc_StatusText( const char *pszName, int iSize, void *pbuf )
{
	...
	strncpy( m_szStatusText[line], READ_STRING(MAX_STATUSTEXT_LENGTH), MAX_STATUSTEXT_LENGTH );
