...
char *CHudTextMessage::BufferedLocaliseTextString( const char *msg )
{
	...
	LocaliseTextString( msg, dst_buffer, sizeof(dst_buffer)-1 );
	return dst_buffer;
}
...
int CHudTextMessage::MsgFunc_TextMsg( const char *pszName, int iSize, void *pbuf )
{
	...
	_snprintf( szBuf[0], sizeof(szBuf[0])-1, "%s\0",  msg_text );

	// keep reading strings and using C format strings for subsituting the strings into the localised text string
	char *sstr1 = LookupString( READ_STRING() );
	_snprintf( szBuf[1], sizeof(szBuf[1])-1, "%s\0",  sstr1 );
	StripEndNewlineFromString( (char*)&szBuf[1] );  // these strings are meant for subsitution into the main strings, so cull the automatic end newlines

	char *sstr2 = LookupString( READ_STRING() );
	_snprintf( szBuf[2], sizeof(szBuf[2])-1, "%s\0",  sstr2 );
	StripEndNewlineFromString( (char*)&szBuf[2] );

	char *sstr3 = LookupString( READ_STRING() );
	_snprintf( szBuf[3], sizeof(szBuf[3])-1, "%s\0",  sstr3 );
	StripEndNewlineFromString( (char*)&szBuf[3] );

	char *sstr4 = LookupString( READ_STRING() );
	_snprintf( szBuf[4], sizeof(szBuf[4])-1, "%s\0",  sstr4 );
	StripEndNewlineFromString( (char*)&szBuf[4] );
	...
	case HUD_PRINTCENTER:
		sprintf( psz, szBuf[0], szBuf[1], szBuf[2], szBuf[3], szBuf[4] );
