...
typedef enum modtype_e {mod_brush, mod_sprite, mod_alias, mod_studio} modtype_t;

playermove_t *pmove = NULL;

//Ben - THESE MUST BE KEPT CONSTANT WITH THE COPIES IN BG_DEFS.H
...
const float STAMINA_LOSS_CROUCH = 10;
//Ben
...
#define CBTEXTURENAMEMAX	21			// only load first n chars of name

#define CHAR_TEX_SNOW	'N'				//texture types
#define CHAR_TEX_SWAMP	'P'
#define CHAR_TEX_MUD	'U'
#define CHAR_TEX_WOOD	'W'
#define CHAR_TEX_SAND	'A'
#define CHAR_TEX_GRASS	'G'
#define CHAR_TEX_GRASS2	'H'
#define CHAR_TEX_LEAVES	'L'
#define CHAR_TEX_DIRT	'D'
#define CHAR_TEX_STONE	'E'
#define CHAR_TEX_CLOTH	'B'
...
#define STEP_GRASS		17		// default step sound
#define STEP_METAL		1		// metal floor
#define STEP_DIRT		2		// dirt
#define	STEP_MUD		3		// mud
#define STEP_SAND		4		// sand
#define STEP_TILE		5		// floor tiles
#define STEP_SLOSH		6		// shallow liquid puddle
#define STEP_WADE		7		// wading in liquid
#define STEP_LADDER		8		// climbing ladder
#define STEP_SWAMP		9		// swamp
#define	STEP_WOOD		10		// walking on wood
#define STEP_LEAVES		11		// leaves
#define STEP_STONE		12		// stone floor
#define	STEP_SNOW		13		// snowy area
#define STEP_CONCRETE	14		// concrete, this shouldn't be used
#define STEP_GRASS2		15		// alt grass sound
#define STEP_CLOTH		16		// for clothes, carpets, etc
...
{
	// Bubble sort, yuck, but this only occurs at startup and it's only 1024 elements...
	//
	int i, j;
...
void PM_PlayStepSound( int step, float fvol )
{
	...
	{
	case STEP_GRASS2:
		sprintf(szSoundPath, "player/pl_grass1");
		break;
	case STEP_GRASS:
		sprintf(szSoundPath, "player/pl_grass");
		break;
	case STEP_MUD:
		sprintf(szSoundPath, "player/pl_mud");
		break;
	case STEP_SAND:
		sprintf(szSoundPath, "player/pl_sand");
		break;
	case STEP_SWAMP:
		sprintf(szSoundPath, "player/pl_swamp");
		break;
	case STEP_WOOD:
		sprintf(szSoundPath, "player/pl_wood");
		break;
	case STEP_LEAVES:
		sprintf(szSoundPath, "player/pl_leaves");
		break;
	case STEP_STONE:
		sprintf(szSoundPath, "player/pl_stone");
		break;
	case STEP_SNOW:
		sprintf(szSoundPath, "player/pl_snow");
		break;
	default:
	case STEP_CONCRETE:
		sprintf(szSoundPath, "player/pl_concrete");
		break;
	case STEP_METAL:
		sprintf(szSoundPath, "player/pl_metal");
		break;
	case STEP_DIRT:
		sprintf(szSoundPath, "player/pl_dirt");
		break;
	case STEP_TILE:
		if (!pmove->RandomLong(0,5) )
			irand = 4+1;
		sprintf(szSoundPath, "player/pl_tile");
		break;
	case STEP_SLOSH:
		sprintf(szSoundPath, "player/pl_slosh");
		break;
	case STEP_WADE:
		sprintf(szSoundPath, "player/pl_wade");
		break;
	case STEP_LADDER:
		sprintf(szSoundPath, "player/pl_ladder");
		break;
	case STEP_CLOTH:
		sprintf(szSoundPath, "player/pl_fabric");
		break;
	}

	irand += 1; // ben - this is nessicary as the random starts at 0 and the sound files start at 1
	sprintf(temp, "%s%i.wav", szSoundPath, irand);

	pmove->PM_PlaySound( CHAN_BODY, temp, fvol, ATTN_NORM, 0, PITCH_NORM );
...
int PM_MapTextureTypeStepType(char chTextureType)
{
	...
	{
		//ben - updated for new material types
		case CHAR_TEX_GRASS:
			return STEP_GRASS;
		case CHAR_TEX_GRASS2:
			return STEP_GRASS2;
		case CHAR_TEX_METAL:
			return STEP_METAL;
		case CHAR_TEX_DIRT:
...
void PM_CatagorizeTextureType( void )
{
	...
	pmove->chtexturetype = CHAR_TEX_STONE;
...
void PM_UpdateStepSound( void )
{
		...
		( speed >= velwalk || !pmove->flTimeStepSound ) )
	{
		...
		else
		{
			...
			{
				//default volume and times for materials which don't require special attention
			default:
				fvol = fWalking ? 0.2 : 0.5;
				pmove->flTimeStepSound = fWalking ? 400 : 300;
				break;
			case STEP_SWAMP:
				...
				pmove->flTimeStepSound = fWalking ? 400 : 300;
				//ben - i hope this works
				break;
			case STEP_WOOD:
				fvol = fWalking ? 0.25 : 0.55;
				pmove->flTimeStepSound = fWalking ? 400 : 300;
				pmove->movevars->friction = 1.0f;
			case STEP_LEAVES:
				fvol = fWalking ? 0.4 : 0.7;
				pmove->flTimeStepSound = fWalking ? 400 : 300;
			case STEP_SNOW:
...
//
	...
	for (i=0 ; i<2 ; i++)
	{
		...
		// Bound it.
		if (pmove->velocity[i] > pmove->clientmaxspeed) 
		{
//			pmove->Con_DPrintf ("PM  Got a velocity too high on %i\n", i);
			pmove->velocity[i] = pmove->clientmaxspeed;
		}
		else if (pmove->velocity[i] < -pmove->clientmaxspeed)
		{
//			pmove->Con_DPrintf ("PM  Got a velocity too low on %i\n", i);
			pmove->velocity[i] = -pmove->clientmaxspeed;
		}
	}
	if (pmove->velocity[2] > pmove->movevars->maxvelocity) 
	{
		pmove->Con_DPrintf ("PM  Got a velocity too high on %i\n", 2);
		pmove->velocity[2] = pmove->movevars->maxvelocity;
	}
	else if (pmove->velocity[2] < -pmove->movevars->maxvelocity)
	{
		pmove->Con_DPrintf ("PM  Got a velocity too low on %i\n", 2);
		pmove->velocity[2] = -pmove->movevars->maxvelocity;
	}
		...
		if ( pmove->cmd.buttons & IN_DUCK )
		{
			...
			if ( pmove->bInDuck )
			{
					 ...
					 ( pmove->onground == -1 ) )
				{
					...
					{
						if(pmove->fuser4 > STAMINA_LOSS_CROUCH)
						{
							pmove->fuser4 -= STAMINA_LOSS_CROUCH;
						}
						else
						{
							pmove->fuser4 = 0;
...
void PM_Jump (void)
{
	int i;
	qboolean tfc = false;
	float flJumpHeightModifier = 0.0f;
	...
	if(pmove->fuser4 < 51)
	{
		flJumpHeightModifier = pmove->fuser4 / MAX_STAMINA;

		flJumpHeightModifier = flJumpHeightModifier > 0.4 ? flJumpHeightModifier : 0.4;
	}
	else
	{
		flJumpHeightModifier = 1;
	}
	...
	if ( ( pmove->bInDuck ) || ( pmove->flags & FL_DUCKING ) )
	{
			...
			Length( pmove->velocity ) > 50 )
		{
			...
			pmove->velocity[2] = (sqrt(2 * 800 * 56.0) * (flJumpHeightModifier > 0.3 ? flJumpHeightModifier : 0.3) );
		}
		else
		{
			pmove->velocity[2] = (sqrt(2 * 800 * 45.0) * (flJumpHeightModifier > 0.3 ? flJumpHeightModifier : 0.3) );
		}
	}
	else
	{
	...
	if(pmove->fuser4 > STAMINA_LOSS_JUMP)
	{
		pmove->fuser4 -= STAMINA_LOSS_JUMP;
	}
	else
	{
		pmove->fuser4 = 0;
...
void PM_CheckParamters( void )
{
	...
	if ( ( spd != 0.0 ) &&
		 ( spd > pmove->clientmaxspeed ) )
	{
		float fRatio = pmove->clientmaxspeed / spd;
...
void PM_PlayerMove ( qboolean server )
{
	...
	if ( pmove->spectator || pmove->iuser1 > 0 )
	{
		...
		//BP export origin for particle blending of smoke
	...
	//BP export movement for crosshair
	VectorCopy( pmove->velocity, export_speed );
	export_frametime = pmove->frametime;
