...
/////////////////////////////////////////////////////////////
//==========================================================
//  PAT: Adding custom player animations HOWTO
//  - Add a new entry in activity.h (ACT_BLAH)
//  - If AI players (monsters) will use it add the SAME enum
//    value in activitymap.h (optional), edit defaultai.cpp also
//  - Add to the enum PLAYER_ANIM in player.h (PLAYER_BLAH)
//  - in the switch(playerAnim) in SetAnimation, make a case statement
//    for this new animation, set the activity defined above there
//  - same function, in switch(m_IdealActivity), add a case statement for it
//  - ALSO, in case ACT_WALK, make an exception for that new ACT_BLAH
//    this is because in halflife, even stationary players are walking with a 
//	pev->velocity of 0.. 
//  - NOTE: for monsters there are several other steps required.
//==========================================================
////////////////////////////////////////////////////////////// 
#include "extdll.h"
#include "util.h"
...
edict_t *EntSelectTeamSpawnPoint( CBasePlayer *pPlayer );
...
int gmsgPredefinedMessage = 0;
int gmsgFullTeamScore = 0;
int gmsgParticles = 0; // particle system
int gmsgGrassParticles = 0; //Grass Entity
//int gmsgArmLegShot = 0; //Stick in our lovely message decleration lol Myristate
int gmsgClockTime = 0;
int gmsgResetScores = 0;
int gmsgColourMessage = 0;
// Ben Addition to Raiko Contrib - Melee Only
int gmsgMeleeOnly = 0;
// Ben End

//Raiko Contrib - Stationary Reload
int gmsgStationaryReload = 0;
//Raiko End
...
void LinkUserMessages( void )
{
	...
	//Ben - Registrations of msgs go here
	gmsgVGUIMenu = REG_USER_MSG("VGUIMenu", 1);
	gmsgTeams = REG_USER_MSG("TeamNames", -1);
	gmsgSpectator = REG_USER_MSG("Spectator", 2);
	gmsgDead = REG_USER_MSG("Dead", -1);
	gmsgPoints = REG_USER_MSG("Points", -1);
	//Ben
	gmsgStatusText = REG_USER_MSG("StatusText", -1);
	gmsgStatusValue = REG_USER_MSG("StatusValue", 3); 
	gmsgResetScores = REG_USER_MSG("ResetScores", -1);
    gmsgClockTime = REG_USER_MSG("ClockTime", -1);
//	gmsgArmLegShot = REG_USER_MSG("ArmLegShot", 1);//Possibly here aswell lol Myristate
	gmsgColourMessage = REG_USER_MSG("Colour", -1);
	gmsgPredefinedMessage = REG_USER_MSG("MakeMessage", -1);
	gmsgFullTeamScore = REG_USER_MSG("FullScore", -1);
	...
	// Ben Addition to Raiko Contrib - Melee Only
	gmsgMeleeOnly = REG_USER_MSG("MeleeOnly", 1);
	// Ben End

	//Raiko Contrib - Stationary Reload
	gmsgStationaryReload = REG_USER_MSG("SReload", 1);
	//Raiko End
	//BP Class statistics hintbox
	gmsgHintbox = REG_USER_MSG( "HintBox", -1 );
	
	//BP Commandersmenu
//	gmsgResComPoint = REG_USER_MSG( "ResComPoint", -1 );
}
...
void CBasePlayer :: TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	if ( pev->takedamage )
	{
		if(pev->flags & FL_CLIENT | FL_FAKECLIENT)
		{
...
//Ben - Removed for demo v1.1
//	// Monkfish - damage decreases stamina by that amount
//	BurnStamina( (flDamage * 5), 1.0 );
	...
	if (fTookDamage && (bitsDamageType & DMG_TIMEBASED) && flHealthPrev < 75.0)
		{
...
// create a box to pack the stuff into.
	...
	pWeaponBox->pev->nextthink = gpGlobals->time + 0.1;
...
{
	if(pev->team == SPECS || m_iDead == DEAD)
	{
		pev->takedamage = DAMAGE_NO;
		return;
	}
	...
	m_flClientHealth = 0.0;
	MESSAGE_BEGIN( MSG_ONE, gmsgHealth, NULL, pev );
		WRITE_BYTE( (int)m_flClientHealth );
	MESSAGE_END();
	...
	// Send Arm Leg Shot Clear
//	MESSAGE_BEGIN( MSG_ONE, gmsgArmLegShot, NULL, pev);
//		WRITE_BYTE(SHOT_CLEAR);
//	MESSAGE_END();
	...
	pKiller = CBaseEntity::Instance(pevAttacker);
...
// PAT: add reload activity/animation
	...
	case ACT_BAYONET:
		if(m_pActiveItem)
		{
			CBaseWeapon *pWeapon = GetClassPtr((CBaseWeapon*)m_pActiveItem->pev);
			if(pWeapon->pData)
			{
				if(pWeapon->pData->iWeaponType != MELEE)
				{
					strcpy( szAnim, "ref_shoot_bayonet" );
				}
				else
				{
	...
	case ACT_RELOAD:
		if(m_pActiveItem)
		{
			...
			if(pWeapon->pData)
			{
				if(pWeapon->pData->iWeaponType == GUN)
				{
					strcpy( szAnim, "reload" );
				}
				else if(pWeapon->pData->iWeaponType == PISTOL)
				{
					strcpy( szAnim, "reloadpistol" );
				}
				//this isn't a gun so no reload
				else
				{
					SetAnimation(PLAYER_IDLE);
					break;
			...
			{
				//no weapon data, so it can't be a gun and therefore no reload
				SetAnimation(PLAYER_IDLE);
				break;
			}
		}
		...
		{
			// no active item, how can we reload no item? :D
			SetAnimation(PLAYER_IDLE);
			break;
		}
	...
	case ACT_WALK:
		...
		if ((m_Activity != ACT_RANGE_ATTACK1 && m_Activity != ACT_RELOAD && m_Activity != ACT_BAYONET) || m_fSequenceFinished)
		{
			if ( FBitSet( pev->flags, FL_DUCKING ) )	// crouching
...
{
	ammo_musket_shots = AmmoInventory( GetAmmoIndex( "MusketShots" ) );
}
...
============
*/
#define AIRTIME	6		// lung full of air lasts this many seconds
...
void CBasePlayer::WaterMove()
{
	...
	if (pev->waterlevel != 3 || pev->watertype <= CONTENT_FLYFIELD) 
	{
		...
		else if (pev->air_finished < gpGlobals->time + ((AIRTIME / 3) * 2))
	...
	{	// fully under water
		...
		if (pev->air_finished < gpGlobals->time)		// drown!
		{
			if (pev->pain_finished < gpGlobals->time)
			{
				...
				// track drowning damage, give half of it back when
				// player finally takes a breath

				m_idrowndmg += (int)(pev->dmg / 2);
			} 
		}
		else
		{
...
void CBasePlayer::PlayerDeathThink(void)
{
	...
	if(m_bIsAllowedToRespawn && m_bHasDied == true)
	{
		extern bool CheckCanJoinClass(CBasePlayer *pPlayer, int iClassNum);

		if(CheckCanJoinClass(this, m_iClassRespawningAs) == false)
			return;

		StopObserver();
		RemoveAllItems(false);
		pev->fuser3 = pev->health = MAX_HEALTH;
		pev->fuser4 = MAX_STAMINA;
		...
		m_bIsAllowedToRespawn = true;

		g_pBGRules->Respawn(this, m_iTeamRespawningAs, m_iClassRespawningAs);

		m_bIsAllowedToRespawn = false;
		pev->nextthink = -1;
		SetThink(NULL);
		return;
	}

	if(!IsObserver() && m_bHasDied == false)
	{
		CopyToBodyQue(pev);

		if(pKiller->edict() == edict()) {
			Observer_FindNextPlayer(false);
		}

		StartObserver(pKiller->pev->origin, pKiller->pev->v_angle);

		pev->deadflag = DEAD_RESPAWNABLE;
		m_bHasDied = true;

		// Make our target our killers
		if(pKiller->IsPlayer() == true) {
			m_hObserverTarget = pKiller;
		}

		// update all those spectating us
		for(int i = 0; i <= gpGlobals->maxClients; i++)
		{
			CBasePlayer *pPlayer = (CBasePlayer*)UTIL_PlayerByIndex(i);
			if(pPlayer && pPlayer->m_hObserverTarget != NULL && pPlayer->m_hObserverTarget->pev == pev && pPlayer != this)
			{
				pPlayer->m_hObserverTarget = m_hObserverTarget;

				if(((pev != pKiller->pev) && pKiller->IsPlayer() == false) || (pev == pKiller->pev)) {
					pPlayer->Observer_SetMode(OBS_ROAMING);
				}
			}
		}

		// killed by world or self so roam
		if(((pev != pKiller->pev) && pKiller->IsPlayer() == false) || (pev == pKiller->pev)) {
			Observer_SetMode(OBS_ROAMING);
		// killed by player, so spec them
		} else {
...
void CBasePlayer::StartDeathCam( void )
{
	...
	pSpot = UTIL_FindEntityByClassname( NULL, "info_player_spectator");	
	...
	if ( pSpot )
	{
		...
		while ( iRand > 0 )
		{
			pNewSpot = UTIL_FindEntityByTargetname( pSpot, "info_player_spectator");
...
//Ben - Removed old Start Observer
...
void CBasePlayer::Jump()
{
	...
	UpdatePlayerSpeedForStamina();

//	m_flNextJumpTime = gpGlobals->time + 1.5;
...
void CBasePlayer::AddPoints( int iDamage, int iCapturePoints )
{
	if( g_fGameOver )
		return;

	m_flCapturePoints += (float)iCapturePoints;
	m_flDamageDone += (float)iDamage;

	
	MESSAGE_BEGIN( MSG_ALL, gmsgScoreInfo );
		WRITE_BYTE( ENTINDEX(edict()) );
		WRITE_LONG( (long)m_flDamageDone);
		WRITE_LONG( (long)m_flCapturePoints );
...
void CBasePlayer::UpdateStatusBar()
{
	...
	if (tr.flFraction != 1.0)
	{
		if ( !FNullEnt( tr.pHit ) )
		{
			...
			if (pEntity->IsPlayer() && (pEntity->pev->team == BRITS || pEntity->pev->team == AMERS))
			{
...
void CBasePlayer::PreThink(void)
{
	...
	if((m_bSpawnGrass == true) && (m_bReSpawnPS == false)) {
		if(m_flLastGrassSpawn < gpGlobals->time) {
			...
			if(pLastGrassSpawned != NULL)
				pLastGrassSpawned->MakeAware( this );
			else
				m_bSpawnGrass = false;

			m_flLastGrassSpawn = gpGlobals->time + 0.2;
		}
	}

	//Ben - Stamina recovery	
	if (m_flStaminaRecoverTime <= gpGlobals->time)
	{
		if(CheckMoving())
		{
			BurnStamina( STAMINA_REGAIN_MOVING/10, 0.1 );
		}
		else
		{
			BurnStamina( STAMINA_REGAIN_STILL/10, 0.1 );
		}
	}
	//Ben
	...
	{
		//Ben - If we are an observer no flashlight	
	...
	if(m_iSpectatorMode != 0)
	{
		//Ben - Added for observer button handling
		if (IsObserver())
		{
...
/*	Ben			if (((i == itbd_NerveGas) && (m_rgbTimeBasedDamage[i] < NERVEGAS_DURATION)) ||
...
// fallpain sound, and damage will be inflicted based on how far the player fell
	...
	//BP commander abilities
//	if((!strcmp(pClass->class_code, "light_a")) || (!strcmp(pClass->class_code, "light_b")))
//		if (!m_bHasDied)
//			pCommander->Think(pev);
		...
		if ( m_rgpPlayerItems[ i ] )
		{
			...
			while ( pPlayerItem )
			{
				...
				if ( gun && gun->UseDecrement() )
				{
					...
					gun->m_fNextStabTime			= max( gun->m_fNextStabTime - gpGlobals->frametime, -0.001 );
...
bool IsSpawnPointValid( CBaseEntity *pPlayer, CBaseEntity *pSpot )
{
	...
	while ( (ent = (CBaseEntity*)UTIL_FindEntityInSphere( ent, pSpot->pev->origin, 32 )) != NULL )
	{
...
bool PlayersInVicinity(CBasePlayer *pPlayer, CBaseEntity *pSpawnSpot)
{
	return true;
}
...
edict_t *EntSelectTeamSpawnPoint( CBasePlayer *pPlayer )
{
	...
	CBasePlayer *pl = (CBasePlayer *)pPlayer; // Get a CBasePlayer

	pSpot = g_pLastSpawn;
	// Randomize the start spot
	for ( int i = RANDOM_LONG(1,5); i > 0; i-- )
	{
		if (pl->pev->team == BRITS) // if BRITISH
			pSpot = UTIL_FindEntityByClassname( pSpot, "info_player_british" );
	    else if (pl->pev->team == AMERS)// if AMERICANS
        	pSpot = UTIL_FindEntityByClassname( pSpot, "info_player_american" );
	    else // not team observer
			pSpot = UTIL_FindEntityByClassname( pSpot, "info_player_spectator" );
	}

    if ( FNullEnt( pSpot ) || pSpot->pev->iuser1 != 1) // skip over the null point and points which are inactive
    {			
		if (pl->pev->team == BRITS)
           pSpot = UTIL_FindEntityByClassname( pSpot, "info_player_british" );
        else if (pl->pev->team == AMERS)
            pSpot = UTIL_FindEntityByClassname( pSpot, "info_player_american" );
        else
            pSpot = UTIL_FindEntityByClassname( pSpot, "info_player_spectator" );
    }
    ...
    do
    {
        if ( pSpot )
        {
            // check if pSpot is valid and active
            if ( IsSpawnPointValid( pPlayer, pSpot ) && pSpot->pev->iuser1 == 1 )
            {
    ...
    if ( !FNullEnt( pSpot ) && pSpot->pev->iuser1 != 0 )
    {
        CBaseEntity *ent = NULL;
...
void CBasePlayer::Spawn( void )
{
	...
	//Ben
	m_flTimeTillNextBattleCry = 0.0;
	m_flTimeTillNextVoiceCommand = 0.0;
	m_iPlayerLevel		= PLAYER;
	m_iDead				= DEAD;
	pev->fuser4			= MAX_STAMINA;
	m_eCurrentMenu		= NO_MENU;
	m_bIsTouchingLbArena= false;
	m_bIsInArena		= false;
	m_bIsAllowedToRespawn = false; // ben - we can alter this depending on which way debugging goes
	m_flLastAmmoStripTime = 0.0;
	m_flLastAmmoGiveTime = 0.0;
	g_engfuncs.pfnSetClientMaxspeed( ENT(pev), 0.00001);
//	ResetClock();//Baer
...
int CBasePlayer::Restore( CRestore &restore )
{
	...
	if ( !pSaveData->fUseLandmark )
	{
		...
		edict_t* pentSpawnSpot = EntSelectTeamSpawnPoint( this );
...
void CBasePlayer::GiveNamedItem( const char *pszName )
{
	...
	//Ben - If we are an observer can't give ourselfes any items
	if ( m_afPhysicsFlags & PFLAG_OBSERVER)
	{
		return;
	}
	...
	GetClassPtr((CBasePlayerItem*)VARS(pent))->AttachToPlayer(this);
}
...
void CBasePlayer :: FlashlightTurnOn( void )
{
	...
	//Ben - If we are an observer we can't turn the flashlight on
...
void CBasePlayer::ImpulseCommands( )
{
	...
	//Ben - If we are an observer no cheats
...
void CBasePlayer :: UpdateClientData( void )
{
	...
	{
		float flHealth = max( pev->health, 0.0 );  // make sure that no negative health values are sent

		// send "health" update message
		MESSAGE_BEGIN( MSG_ONE, gmsgHealth, NULL, pev );
			WRITE_BYTE( (int)flHealth );
		MESSAGE_END();

		m_flClientHealth = flHealth;
	}
...
//Ben - All player functions go here
...
bool CBasePlayer::CheckMoving()
{
	if((pev->velocity.x != 0.0) || (pev->velocity.y != 0.0) || (pev->velocity.z != 0.0))
	{
		return true;
	}
	else
...
void CBasePlayer::ResetSpeed()
{
	if(g_pBGRules->pRoundRules->GetRoundState() == ROUND_PAUSED)
		return;

	float flSpeed;
	
	if(m_fInRush)
		flSpeed = iClassSpeeds[pev->team][pev->playerclass] * RUSH_SPEEDMODIFIER;
	else
		flSpeed = iClassSpeeds[pev->team][pev->playerclass];
	
	CBasePlayerWeapon *pGun;

	if(m_pActiveItem)
	{
		pGun = GetClassPtr((CBasePlayerWeapon*)m_pActiveItem->pev);

		if(pGun->pData)
		{
			flSpeed -= pGun->pData->flSpeedModifier;
		}
	}
	g_engfuncs.pfnSetClientMaxspeed( edict(), flSpeed);
}

//adds or removes(burns) stamina
bool CBasePlayer::BurnStamina(float flStamina, float flTimeToNextBurn)
{
	//we can't have instant stamina
	if(!flTimeToNextBurn)
	{
		flTimeToNextBurn = 1.0;
	}
	//is there enough stamina to burn
	if(CanBurnMoreStamina(flStamina) && (flStamina > 0))
	{ 
		pev->fuser4 -= flStamina;
		m_flStaminaRecoverTime = gpGlobals->time + flTimeToNextBurn;
		UpdatePlayerSpeedForStamina();
		return true;
	}
	//we are trying to add on stamina
	else if((flStamina < 0))
	{
		//enough stamina empty, to do our readd stamina
		if((pev->fuser4 - flStamina) < MAX_STAMINA)
		{
			// regains stamina
			pev->fuser4 -= flStamina;
		}
		//not enough stamina been used up, so the stamina will go to the max
		else
		{
			pev->fuser4 = MAX_STAMINA;
		}

		// PAT: clamp to max of current health
		// NOTE: for whatever reason BG's health goes from 0 - 20
		//       and the stamina goes from 0 - 100 :( Need this fudge
		//       factor.
		// Ben - MAX_STAMINA/MAX_HEALTH

		// if (pev->fuser4 > (pev->health * MAX_STAMINA/MAX_HEALTH))
		//		pev->fuser4 = (pev->health * MAX_STAMINA/MAX_HEALTH);

		// Ben - The clamping is now locked to half of the max stamina plus 1 (51 atm)

		// Equation for this is
		/*
		Lowest_Stamina_Value = Normal_Max_Stamina / 2 + 1;

		Current_Max_Stamina = ((((Normal_Max_Stamina / 2 ) / Max_Health) * health)
								* (Normal_Max_Stamina / (Lowest_Stamina_Value * 2)))
								+ Lowest_Stamina_Value;

		Further details can be found at this thread - http://www.bgmod.com/forums/index.php?s=&act=ST&f=1&t=835&st=0&#entry10732
		*/

		/* ben - removed for the demo v1.1
		float scaled_stamina = ((MAX_STAMINA / 2) / MAX_HEALTH);
		float stamina_low = (MAX_STAMINA / 2) + 1;
		float stamina_correction = MAX_STAMINA / (stamina_low * 2);

		float current_max_stamina = ((scaled_stamina * pev->health) * stamina_correction) + stamina_low;

		if(pev->fuser4 > current_max_stamina)
			pev->fuser4 = current_max_stamina; 
		*/
	...
	//trying to use more stamina, keep the palyer waiting, as they are attemping it
	else if(CanBurnMoreStamina(flStamina) == false)
	{
		m_flStaminaRecoverTime = gpGlobals->time + flTimeToNextBurn;
		return false;
	}
	//dunno why i did this, it seems cleaner i suppose
	else
	{
		m_flStaminaRecoverTime = gpGlobals->time + flTimeToNextBurn;
		return false;
...
//do we have enough stamina left to remove it or wil lthe action have to be canceled
bool CBasePlayer::CanBurnMoreStamina(float flStamina)
{
	if((pev->fuser4 - flStamina) >= 0)
	{
		return true;
	}
	else
...
void CBasePlayer::UpdatePlayerSpeedForStamina()
{
	if(g_pBGRules->pRoundRules->GetRoundState() == ROUND_PAUSED)
		return;

	float flModifier = pev->fuser4 / MAX_STAMINA;

	flModifier = flModifier > 0.2 ? flModifier : 0.2; //sets the lowest amount of stamina that can affect palyer speeds
	flModifier = 1 - ((1 - flModifier) / 2.5); //reduces the amount of speed that is reduced :D
	...
	if(m_pActiveItem)
	{
		...
		// half speed if we are reloading
		if(pGun->m_fInReload == true)
		{
			// Raiko Contrib - Stationary Reload
			if(CVAR_GET_FLOAT("mp_stationaryreload") > 0.0)
				flModifier *= 0.0;
			// Raiko End
	...
	if (m_flControllingCannon)
	{
		// drain all stamina so we cannot jump while using the cannon
		BurnStamina( 1, 0.1 );
		// make the player unable to move while using the cannon
		flModifier *= 0.0;
	}
	flSpeed = flSpeed * flModifier;

	
	if(pev->iuser1 != UNDEFINED && ((int)CVAR_GET_FLOAT("mp_forcechasecam") >= 2)) {
		flSpeed = 0;
	}
