...
/*	else if(FStrEq(pcmd, "addbot"))
	{
		char *pszInfo = NULL;
		char szRejection[256];
		int iIndex = 0;
		edict_t *pBotEdict = g_engfuncs.pfnCreateFakeClient("Bot");
		CBasePlayer *pBot = GetClassPtr((CBasePlayer*)VARS(pBotEdict));
		pBotEdict = pBot->edict();

		ClientConnect(pBotEdict, "Bot", "127.0.0.1", szRejection);
		ClientPutInServer(pBotEdict);
		pBot->pev->flags |= FL_FAKECLIENT;
		g_pBGRules->HandleTeams(pBot, 1);
		g_pBGRules->HandleClasses(pBot, 2);

		pszInfo = g_engfuncs.pfnGetInfoKeyBuffer(pBotEdict);
		ClientUserInfoChanged(pBotEdict, pszInfo);
	} */
	//Ben
