...
DLL_GLOBAL	int	g_sModelIndexLaser;// holds the index for the laser beam
DLL_GLOBAL  const char *g_pModelNameLaser = "sprites/laserbeam.spr";
DLL_GLOBAL	int	g_sModelIndexLaserDot;// holds the index for the laser beam dot
DLL_GLOBAL	int	g_sModelIndexFireball;// holds the index for the fireball
DLL_GLOBAL	int	g_sModelIndexSmoke;// holds the index for the smoke cloud
DLL_GLOBAL	int	g_sModelIndexWExplosion;// holds the index for the underwater explosion
DLL_GLOBAL	int	g_sModelIndexBubbles;// holds the index for the bubbles model
DLL_GLOBAL	int	g_sModelIndexBloodDrop;// holds the sprite index for the initial blood
DLL_GLOBAL	int	g_sModelIndexBloodSpray;// holds the sprite index for splattered blood
...
void W_Precache(void)
{
	...
	UTIL_PrecacheOtherWeapon( "weapon_BrownBess" );
	UTIL_PrecacheOtherWeapon( "weapon_Revolutionnaire" );
	UTIL_PrecacheOtherWeapon( "weapon_Charleville" );
	UTIL_PrecacheOtherWeapon( "weapon_Pennsylvania" );

	UTIL_PrecacheOtherWeapon( "weapon_PistolA" );
	UTIL_PrecacheOtherWeapon( "weapon_PistolB" );

	UTIL_PrecacheOtherWeapon( "weapon_Sabre" );
	...
	//Ben - Smoke Sprite
	g_sModelIndexSmoke = PRECACHE_MODEL ("sprites/gfx_musketsmoke.spr");// smoke
	g_sModelIndexSmoke = PRECACHE_MODEL ("sprites/gfx_smoke.spr");// smoke
...
void CBasePlayerItem :: FallInit( void )
{
	...
	pev->nextthink = gpGlobals->time + (float)0.1;
}
...
//Ben - Stores the damage sent to the function
void CBaseEntity::SetWeaponDamage(float flDamage)
{
	m_flSkillModifier = flDamage;
}
...
void CBasePlayerWeapon::ItemPostFrame( void )
{
	...
	else if ( !(m_pPlayer->pev->button & (IN_ATTACK|IN_ATTACK2) ) )
	{
		...
		if ( !IsUseable() && m_flNextPrimaryAttack < ( UseDecrement() ? 0.0 : gpGlobals->time ) ) 
		{
			...
			{
				m_flNextPrimaryAttack = ( UseDecrement() ? 0.0 : gpGlobals->time ) + (float)0.3;
				return;
			}
		}
		else
		{
			...
			{
				ALERT(at_console, "Server dll reload2\n");
				Reload();
				return;
			}
			*/
...
//Ben		EMIT_SOUND(ENT(pev), CHAN_ITEM, "items/9mmclip1.wav", 1, ATTN_NORM);
		}
	}
