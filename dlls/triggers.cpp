...
{
	if((pev->team != pCaller->pev->team) && (pev->team != UNDEFINED))
		return;
...
void CBaseTrigger :: HurtTouch ( CBaseEntity *pOther )
{
	...
	if((pev->team != pOther->pev->team) && (pev->team != UNDEFINED))
		return;
...
public:
	void Spawn( void );
	void EXPORT TriggerUse(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE iUseType, float flValue) { if(pActivator) SUB_UseTargets( pActivator, iUseType, flValue ); }
};
...
//	else
		...
		SetUse( &CTriggerMultiple::TriggerUse );
	}
...
void CBaseTrigger :: MultiTouch( CBaseEntity *pOther )
{
	...
	if ( ((pevToucher->flags & FL_CLIENT) && !(pev->spawnflags & SF_TRIGGER_NOCLIENTS)) ||
		...
		((pev->team == pevToucher->team) || (pev->team == UNDEFINED)))
	{
...
void CBaseTrigger::CounterUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if((pev->team != pActivator->pev->team) && (pev->team != UNDEFINED))
		return;
...
void CTriggerPush :: Touch( CBaseEntity *pOther )
{
	...
	if((pev->team != pevToucher->team) && (pev->team != UNDEFINED))
		return;
