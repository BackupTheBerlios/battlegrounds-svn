...
void CBaseBgRules::ClientUserInfoChanged( CBasePlayer *pPlayer, char *infobuffer )
{
	...
	if(pPlayer->pev->team && pPlayer->pev->playerclass)
	{
		sprintf(temp, "%s", szClassCodes[pPlayer->pev->team][pPlayer->pev->playerclass]);
	}
	else if(pPlayer->m_iTeamRespawningAs && pPlayer->m_iClassRespawningAs)
	{
		sprintf(temp, "%s", szClassCodes[pPlayer->m_iTeamRespawningAs][pPlayer->m_iClassRespawningAs]);
	}
	else
	{
		sprintf(temp, "%s", szClassCodes[BRITS][UNDEFINED+1]);
		pPlayer->pev->effects |= EF_NODRAW;
	}

	g_engfuncs.pfnSetClientKeyValue( pPlayer->entindex(), g_engfuncs.pfnGetInfoKeyBuffer( pPlayer->edict() ), "model", (char*)temp);

	SET_MODEL( ENT(pPlayer->pev), "models/player.mdl");		
	g_ulModelIndexPlayer = pPlayer->pev->modelindex;
