...
// CGameScore / game_score	-- award points to player
//	Points +/- total
...
void CGameScore::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	{
		pActivator->AddPoints( Points(), 0 );
	}
}

// CGameTeamScore / game_score	-- award points to team 
//	Points +/- total
//	Flag: Allow negative scores					SF_SCORE_NEGATIVE
...
class CGameTeamScore : public CRulePointEntity
{
public:
	void	Spawn( void );
	void	Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value );
	...
	inline	int		Points( void ) { return (int)pev->frags; }
	inline	int		Team( void ) { return pev->team; }
	inline	bool	AllowNegativeScore( void ) { return pev->spawnflags & SF_SCORE_NEGATIVE; }

	inline	void	SetPoints( int points ) { pev->frags = points; }
	inline	void	SetTeam( int team ) { pev->team = team; }

private:
};

LINK_ENTITY_TO_CLASS( game_team_score, CGameTeamScore );


void CGameTeamScore::Spawn( void )
{
	CRulePointEntity::Spawn();
}


void CGameTeamScore::KeyValue( KeyValueData *pkvd )
{
	if (FStrEq(pkvd->szKeyName, "points"))
	{
		SetPoints( atoi(pkvd->szValue) );
...
void CGameTeamScore::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	if ( !CanFireForActivator( pActivator ) )
		return;
	...
	if(Team() == UNDEFINED)
	{
		g_pBGRules->GiveTeamPoints( BRITS, Points() );
		g_pBGRules->GiveTeamPoints( AMERS, Points() );
	}
	else
	{
		g_pBGRules->GiveTeamPoints( Team(), Points() );
	}
}
...
void CGameText::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	else
	{
		if ( pActivator->IsNetClient() )
				MESSAGE_BEGIN( MSG_ONE, SVC_TEMPENTITY, NULL, pActivator->edict() );
		else
			return;
	}

	WRITE_BYTE( TE_TEXTMESSAGE );
	WRITE_BYTE( m_textParms.channel & 0xFF );

	WRITE_SHORT( FixedSigned16( m_textParms.x, 1<<13 ) );
	WRITE_SHORT( FixedSigned16( m_textParms.y, 1<<13 ) );
	WRITE_BYTE( m_textParms.effect );

	WRITE_BYTE( m_textParms.r1 );
	WRITE_BYTE( m_textParms.g1 );
	WRITE_BYTE( m_textParms.b1 );
	WRITE_BYTE( m_textParms.a1 );

	WRITE_BYTE( m_textParms.r2 );
	WRITE_BYTE( m_textParms.g2 );
	WRITE_BYTE( m_textParms.b2 );
	WRITE_BYTE( m_textParms.a2 );

	WRITE_SHORT( FixedUnsigned16( m_textParms.fadeinTime, 1<<8 ) );
	WRITE_SHORT( FixedUnsigned16( m_textParms.fadeoutTime, 1<<8 ) );
	WRITE_SHORT( FixedUnsigned16( m_textParms.holdTime, 1<<8 ) );

	if ( m_textParms.effect == 2 )
		WRITE_SHORT( FixedUnsigned16( m_textParms.fxTime, 1<<8 ) );
	
	if ( strlen( MessageGet() ) < 512 )
	{
		WRITE_STRING( MessageGet() );
	}
	else
	{
		char tmp[512];
		strncpy( tmp, MessageGet(), 511 );
		tmp[511] = '\0';
		WRITE_STRING( tmp );
	}
...
const char *CGameTeamMaster::TeamID( void )
{
	...
	return szTeamNames[m_teamIndex];
}
