...
// ben - distance between 2 vectors
Vector UTIL_DistanceBetween(Vector v1, Vector v2)
{
...
void UTIL_HudMessage( CBaseEntity *pEntity, const hudtextparms_t &textparms, const char *pMessage )
{
	...
	extern int gmsgColourMessage;
	MESSAGE_BEGIN( MSG_ONE, gmsgColourMessage, NULL, pEntity->edict());
		WRITE_SHORT( (int)textparms.x );
		WRITE_SHORT( (int)textparms.y );

		WRITE_SHORT( textparms.r1 );
		WRITE_SHORT( textparms.g1 );
		WRITE_SHORT( textparms.b1 );

		char tmp[256];
		strncpy( tmp, pMessage, 255 );
		tmp[255] = '\0';
		WRITE_STRING( tmp );

		WRITE_SHORT( (short)textparms.holdTime);
	MESSAGE_END();
...
// Preceded by LOG: ( timestamp ) < message >
//=========================================================
void UTIL_LogPrintf( LOG_LEVEL iLogLevel, char *fmt, ... )
{
	if((int)CVAR_GET_FLOAT("mp_logdetail") < (int)iLogLevel)
		return;
