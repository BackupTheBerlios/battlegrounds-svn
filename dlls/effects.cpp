...
void SendFogData ( Vector col, int iFadeTime, int iStartDist, int iEndDist );
Vector g_col;
int g_iFadeOut;
int g_iStartDist;
int g_iEndDist;
...
void CBubbling::Spawn( void )
{
	...
	int speed = (int)pev->speed > 0 ? (int)pev->speed : -(int)pev->speed;
...
void CLaser::Spawn( void )
{
	...
	if ( m_pSprite )
		m_pSprite->SetTransparency( kRenderGlow, (int)pev->rendercolor.x, (int)pev->rendercolor.y, (int)pev->rendercolor.z, (int)pev->renderamt, pev->renderfx );

	if ( pev->targetname && !(pev->spawnflags & SF_BEAM_STARTON) )
	{
		TurnOff();
		m_iDefaultState = USE_OFF;
	}
	else
	{
		TurnOn();
		m_iDefaultState = USE_ON;
	}
}
...
{
	if (!ShouldToggle(useType, !!(m_state))) return;
...
//	ALERT(at_console,"RainThink offs.z = %f, size.z = %f\n",vecOffs.z,pev->size.z);
	...
	while (drawn < repeats && tries < (repeats*3))
	{
		...
		if (m_axis == AXIS_X)
			...
			vecSrc.x = pev->mins.x + RANDOM_LONG(0, (long)pev->size.x);
		if (m_axis == AXIS_Y)
			vecSrc.y = pev->maxs.y;
		else
			vecSrc.y = pev->mins.y + RANDOM_LONG(0, (long)pev->size.y);
		if (m_axis == AXIS_Z)
			vecSrc.z = pev->maxs.z;
		else
			vecSrc.z = pev->mins.z + RANDOM_LONG(0, (long)pev->size.z);
...
//		vecDest.z = pev->mins.z;
	...
	if (m_flMaxUpdateTime)
		pev->nextthink = (gpGlobals->time + RANDOM_FLOAT(m_flMaxUpdateTime, m_flUpdateTime) );
...
{
	ALERT(at_console, "Fog Spawn Called\n");
	pev->effects |= EF_NODRAW;
	...
	if (pev->spawnflags & SF_FOG_ACTIVE)
	{
		...
		pev->nextthink = gpGlobals->time + 3; // give the client time to start up, hopefully not too much.
	}
...
void CEnvFog :: TurnOn ( void )
{
	...
	if( m_iFadeIn )
	{
		...
		g_col = pev->rendercolor;
		g_iFadeOut = m_iFadeIn;
		g_iStartDist = m_iStartDist;
		g_iEndDist = m_iEndDist;
		
		SendFogData( pev->rendercolor, m_iFadeIn, m_iStartDist, m_iEndDist);
	...
	else
	{
		...
		SendFogData( pev->rendercolor, 0, m_iStartDist, m_iEndDist);
		if (m_fHoldTime)
		{
...
void CEnvFog :: TurnOff ( void )
{
	...
	if( m_iFadeOut )
	{
		...
		g_col = (Vector)pev->rendercolor;
		g_iFadeOut = (-m_iFadeOut);
		g_iStartDist = m_iStartDist;
		g_iEndDist = m_iEndDist;

		SendFogData( pev->rendercolor, -m_iFadeOut, m_iStartDist, m_iEndDist);
...
void SendFogData ( Vector col, int iFadeTime, int iStartDist, int iEndDist )
{
	...
	WRITE_BYTE ( (int)col.x );
	WRITE_BYTE ( (int)col.y );
	WRITE_BYTE ( (int)col.z );
