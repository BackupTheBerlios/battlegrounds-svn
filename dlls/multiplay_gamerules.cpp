...
//Ben - Damage Amounts
	gSkillData.plrDmgPistol = 2;

	gSkillData.plrDmgMusket = 4;

	gSkillData.plrDmgRifle = 3;

	gSkillData.plrDmgKnife = 7;

	gSkillData.plrDmgSabre = 9;

	gSkillData.plrDmgBandages = -2;
}
...
//	pl->m_iHideHUD |= HIDEHUD_ALL; 

	g_pBGRules->GetPlayerSpawnSpot(pl)	;
	...
	if ( pClient )
	{
		...
		if ( pPlayer )
		{
			if(pPointManager != NULL)
				pPointManager->PlayerDisconnected(pPlayer);

			//remove them from the line battle arena list
//			extern void RemovePlayer(CBasePlayer *pPlayer);
//			RemovePlayer(pPlayer);
			...
			//Ben - Tell all clients this player isn't a spectator anymore
			...
			pPlayer->m_iTeamRespawningAs = UNDEFINED;
			pPlayer->pev->playerclass = UNDEFINED;
			pPlayer->pev->team = UNDEFINED;

			// ben - addin event for point manager
		}
	}
}
...
float CHalfLifeMultiplay :: FlPlayerFallDamage( CBasePlayer *pPlayer )
{
	...
	switch ( iFallDamage )
	{
		case 1://progressive
		{
			...
			float flDamage = pPlayer->m_flFallVelocity * DAMAGE_FOR_FALL_SPEED;
			return flDamage;
			break;
		}
		default:
...
void CHalfLifeMultiplay :: PlayerKilled( CBasePlayer *pVictim, entvars_t *pKiller, entvars_t *pInflictor )
{
	...
	MESSAGE_BEGIN( MSG_ALL, gmsgScoreInfo );
		...
		WRITE_LONG( (long)pVictim->m_flDamageDone );
		WRITE_LONG( (long)pVictim->m_flCapturePoints );
	...
	pVictim->m_iDead = DEAD;

	MESSAGE_BEGIN( MSG_ALL, gmsgDead);
		WRITE_BYTE(ENTINDEX(pVictim->edict()));
		WRITE_BYTE(pVictim->m_iDead);
	MESSAGE_END();
...
//	CBaseEntity *Killer = CBaseEntity::Instance( pKiller );
	...
	MESSAGE_BEGIN( MSG_ALL, gmsgDeathMsg );
		...
		if ( (pKiller != pVictim->pev) && (PlayerRelationship( pVictim, GetClassPtr((CBasePlayer*)pKiller) ) == GR_TEAMMATE) )
	...
	else if ( pKiller->flags & FL_CLIENT )
	{
		...
		if(pWeapon && pWeapon->pData && !(strcmp(STRING(pevInflictor->classname), "weapon_Bayonet")))
		{
			...
			UTIL_LogPrintf( "\"%s<%i><%s><%s>\" killed \"%s<%i><%s><%s>\" with \"%s_bayonet\"\n",  
				STRING( pKiller->netname ),
				GETPLAYERUSERID( ENT(pKiller) ),
				GETPLAYERAUTHID( ENT(pKiller) ),
				szTeamNames[pKiller->team],
				STRING( pVictim->pev->netname ),
				GETPLAYERUSERID( pVictim->edict() ),
				GETPLAYERAUTHID( pVictim->edict() ),
				szTeamNames[pVictim->pev->team],
