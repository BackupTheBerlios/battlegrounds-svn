...
protected:
	...
	Vector		angles;			// BP needed to controll the cannon with movebuttons
	float		m_NextCannonHudUpdate; // so we don't have to spam the client using the cannon with text
};
...
void CFuncTank :: Spawn( void )
{
	...
	angles.y = m_yawCenter;
	angles.x = m_pitchCenter;

	pev->oldorigin = pev->origin;
	pev->classname = MAKE_STRING("Cannon");
}
...
//	ALERT(at_console,"StartControl succeeded\n");
	return true;
}
...
void CFuncTank::TrackTarget( void )
{
	...
	Vector direction, targetPosition, barrelEnd;
	edict_t *pTarget = NULL;

	// no movement when controlling a cannon
	m_pController->m_flControllingCannon = true;

	// Get a position to aim for
	if (m_pController)
	{
		//old:  Tanks attempt to mirror the player's angles
		
		//new: player uses the movement keys to rotate the cannon
		if (m_pController->pev->button & IN_FORWARD)
			angles[0]--;
		if (m_pController->pev->button & IN_BACK)
			angles[0]++;
		if (m_pController->pev->button & IN_MOVELEFT)
			angles[1]++;
		if (m_pController->pev->button & IN_MOVERIGHT)
			angles[1]--;
		
		if (m_pController->pev->button & IN_USE)
		{
			m_pController->m_flControllingCannon = false;
			StopControl();
			return;
		}
...
//		direction = m_sightOrigin - barrelEnd;
	...
	if (gpGlobals->time > m_NextCannonHudUpdate)
	{
		// draw cannon pitch / rotation details 
		char cannonangles[128];
		int tempangle = -(int)angles[0];
		sprintf (cannonangles, "Cannon Controlls:\n \nPitch: %d  Rotation: %d", tempangle, (int)angles[1]);
		ClientPrint( m_pController->pev,  HUD_PRINTCENTER, cannonangles );
		m_NextCannonHudUpdate = gpGlobals->time + 0.25;
	}
...
void CFuncTankGun::Fire( const Vector &barrelEnd, const Vector &forward, entvars_t *pevAttacker )
{
	...
	if ( m_fireLast != 0 )
	{
		...
		if ( bulletCount > 0 )
		{
			...
			CGrenade::ShootContact( pevAttacker, forward, forward * 800 );
...
{
	pev->classname = MAKE_STRING("Cannon");
	GetClassPtr((CBasePlayer*)pevAttacker)->m_flNextAttack = gpGlobals->time;
...
//	ALERT(at_console, "FuncTankMortar::Fire\n");
	if ( m_fireLast != 0 )
	{
		...
		if ( bulletCount > 0 )
		{
			...
			CGrenade::ShootTimed( pevAttacker, barrelEnd , forward * 2000, 1.3f );
			// old: CFuncTank::Fire( barrelEnd, forward, pev );
			
			// todo!!!
			// add cannon muzzle flash and smoke particle system
		}
	}
	else
		CGrenade::ShootContact( pevAttacker, barrelEnd, forward * 2000 );
		//old: CFuncTank::Fire( barrelEnd, forward, pev );
}
...
void CFuncTankControls :: Think( void )
{
	...
	} while ( !FNullEnt(pTarget) && strncmp( STRING(pTarget->v.classname), "Cannon", 6 ) );
