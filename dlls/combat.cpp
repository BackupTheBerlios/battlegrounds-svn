...
//extern int gmsgArmLegShot;	//Dont forget to let give ur calls below access
...
int CBaseMonster :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	// --add the damage to the attacker
	CBaseEntity *pPlayer = GetClassPtr((CBaseEntity*)pevAttacker);
	if(pPlayer->IsPlayer())
	{
		// --if the player is himself do the damge points negatively
		if(pevAttacker == pev)
			pPlayer->AddPoints(-(int)flTake, 0);

		// --if the 2 players are on the same team, negative points
		else if(pevAttacker->team == pev->team)
			pPlayer->AddPoints(-(int)flTake, 0);

		else
			pPlayer->AddPoints(((int)flTake), 0);

		// if both parties involved are players, log the damage done
		if(this->IsPlayer())
		{
			...
			else if ( strncmp( killer_weapon_name, "funk_tankmortar", 15) == 0 )
				killer_weapon_name = "Cannon";
			...
			{
				killer_weapon_name = STRING(pWeapon->pData->classname);
				killer_weapon_name += 7;
				sprintf(szWeaponName, "%s_bayonet", killer_weapon_name);
			}
			else
				sprintf(szWeaponName, "%s", killer_weapon_name);

			UTIL_LogPrintf(DAMAGE_LOGGING, "\"%s<%i><%s><%s>\" attacked \"%s<%i><%s><%s>\" with \"%s\" (damage \"%i\") (hitbox \"%s\")\n", 
				...
				szTeamNames[pev->team],
				szWeaponName,
				(int)flTake,
				szHitBox[pevInflictor->iuser3]);
		}
	}
	...
	while ((pEntity = UTIL_FindEntityInSphere( pEntity, vecSrc, flRadius )) != NULL)
	{
		if ( pEntity->pev->takedamage != DAMAGE_NO )
		{
			...
			{// the explosion can 'see' this entity, so hurt them!
				...
				if (tr.flFraction != 1.0)
				{
					if(pEntity->pev->flags & FL_CLIENT | FL_FAKECLIENT)
					{
						ClearMultiDamage( );
...
void CBaseEntity::TraceAttack(entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	...
	if ( pev->takedamage )
	{
		...
		if ( blood != DONT_BLEED || bitsDamageType == DMG_BULLET)
		{
...
void CBaseMonster :: TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	unsigned int ArmLegShot = 0; //Myristate
	...
	if ( pev->takedamage )
	{
		...
		case HITGROUP_GENERIC:
          ...
          case HITGROUP_STOMACH: 
               ...
               break;
          case HITGROUP_LEFTARM:
               ArmLegShot = SHOT_LEFT_ARM;
               flDamage *= gSkillData.plrArm; 
               break;
          case HITGROUP_RIGHTARM:
               ArmLegShot = SHOT_RIGHT_ARM;
               flDamage *= gSkillData.plrArm;
               break;
          case HITGROUP_LEFTLEG: 
               ArmLegShot = SHOT_LEFT_LEG;
               flDamage *= gSkillData.plrLeg;
               break;
          case HITGROUP_RIGHTLEG: 
               ArmLegShot = SHOT_RIGHT_LEG;
               flDamage *= gSkillData.plrLeg; 
               break;
	...
	for (ULONG iShot = 1; iShot <= cShots; iShot++)
	{
		...
		if (tr.flFraction != 1.0)
		{
			...
			case BULLET_NONE: // FIX 
				pEntity->TraceAttack(pevAttacker, 50, vecDir, &tr, DMG_CLUB | DMG_NEVERGIB );
				TEXTURETYPE_PlaySound(&tr, vecSrc, vecEnd, iBulletType);
				// only decal glass
...
================
*/
Vector CBaseEntity::FireBulletsPlayer ( ULONG cShots, Vector vecSrc, Vector vecDirShooting, Vector vecSpread, float flDistance, int iBulletType, int iTracerFreq, int iDamage, entvars_t *pevAttacker, entvars_t *pevInflictor, int shared_rand )
{
	static int tracerCount;
	TraceResult tr;
	Vector vecRight = gpGlobals->v_right;
	...
	for ( ULONG iShot = 1; iShot <= cShots; iShot++ )
	{
		...
		if (tr.flFraction != 1.0)
		{
			...
			//BP: Damage depending on distance the bullet travels
			float deltaX = vecSrc[0] - tr.vecEndPos[0];
			float deltaY = vecSrc[1] - tr.vecEndPos[1];
			float deltaZ = vecSrc[2] - tr.vecEndPos[2];
			
			float ShotDistance = sqrt ((deltaX * deltaX) + (deltaY * deltaY) + (deltaZ * deltaZ));
			...
			default:
			case BULLET_SHOT:
				flDamage = GetDamage(pevAttacker, vecDir, &tr, ShotDistance);
				...
				{
					// Make a lot of Blood!
					float bloodamount = flDamage;
					SpawnBlood( tr.vecEndPos, BLOOD_COLOR_RED, bloodamount); 

					// and some bloodstreams as well
					vec3_t stream1;

					stream1[0] = tr.vecPlaneNormal[0] + RANDOM_FLOAT( -0.2f, 0.2f );
					stream1[1] = tr.vecPlaneNormal[1] + RANDOM_FLOAT( -0.2f, 0.2f );
					stream1[2] = tr.vecPlaneNormal[2] + RANDOM_FLOAT( -0.2f, 0.2f );

					UTIL_BloodStream( tr.vecEndPos, tr.vecPlaneNormal, 75, bloodamount );
					UTIL_BloodStream( tr.vecEndPos, stream1, 75, bloodamount );
				}
				break;
			}
		}
	...
	pevInflictor->iuser3 = tr.iHitgroup;
	ApplyMultiDamage(pevInflictor, pevAttacker);
...
*/
	...
	for ( i = 0 ; i < cCount ; i++ )
	{
		...
		UTIL_TraceLine( ptr->vecEndPos, ptr->vecEndPos + vecTraceDir * -300, ignore_monsters, ENT(pev), &Bloodtr);
...
float CBaseEntity::GetDamage(entvars_t *pevAttacker, Vector vecDir, TraceResult *ptr, float BulletTravelDistance)
{
	float flDamageMultiplier = GetClassPtr((CBasePlayer*)pevAttacker)->m_pActiveItem->m_flSkillModifier;
	float flDamage = 0;

	edict_t *pAttackingPlr = NULL;

	if(pev->owner)
		pAttackingPlr = pev->owner;
	else if(edict())
		pAttackingPlr = edict();
	else
		pAttackingPlr = NULL;

	if(VARS(ptr->pHit)->takedamage == DAMAGE_NO)
		return 0.0;

	CBaseEntity *pVictim = GetClassPtr((CBaseEntity*)VARS(ptr->pHit));
	CBaseEntity *pAttacker = GetClassPtr((CBaseEntity*)VARS(pAttackingPlr));

	//Ben - Damage multiplier
	switch ( ptr->iHitgroup )
	{
	case HITGROUP_GENERIC:
		flDamage = HIT_GENERIC_DMG;
		if(pVictim->IsPlayer())
			return 0.0;
		break;

	case HITGROUP_HEAD:
		flDamage = HIT_HEAD_DMG;
		break;

	case HITGROUP_CHEST:
		flDamage = HIT_CHEST_DMG;
		break;

	case HITGROUP_STOMACH:
		flDamage = HIT_STOMACH_DMG;
		break;

	case HITGROUP_LEFTARM:
		flDamage = HIT_ARM_DMG;
		break;
	...
	default:
		flDamage = MAX_HEALTH / 2;
		break;
	}

	extern int gmsgPredefinedMessage;
	extern DLL_GLOBAL CGameRules	*g_pGameRules;
	bool bTakeDamage = g_pGameRules->FPlayerCanTakeDamage((CBasePlayer*)pVictim, (CBasePlayer*)pAttacker);

	flDamage *= flDamageMultiplier;
	
	// if we haven't set a distance then ignore distance based damage
	// if we haven't travelled 800 units then use normal power
	if(BulletTravelDistance >= 800.0) {
		// ramp between 1 and 0.3 between 800 and 3000 units
		if(BulletTravelDistance <= 3000) {
			float flDistanceModifier = ((3800 - BulletTravelDistance) / 3000);
			flDamage *= flDistanceModifier;
		// use 0.3 damage at 3000 and over
		} else {
			flDamage *= 0.3;
		}
	}

	if(pAttacker && pVictim && pVictim->pev->netname && bTakeDamage != false 
		&& pAttacker->IsPlayer() && pVictim->IsPlayer())
	{
		MESSAGE_BEGIN(MSG_ONE, gmsgPredefinedMessage, NULL, pAttacker->pev);

		WRITE_SHORT(iHitMsg_Attacker);
		...
		MESSAGE_BEGIN(MSG_ONE, gmsgPredefinedMessage, NULL, pVictim->pev);

		WRITE_SHORT(iHitMsg_Victim);
		WRITE_SHORT(ptr->iHitgroup);
		WRITE_SHORT(pAttacker->entindex());
		WRITE_SHORT(flDamage);
