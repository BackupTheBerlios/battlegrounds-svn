...
//				ALERT( at_console, "Added global entity %s (%s)\n", STRING(pEntity->pev->classname), STRING(pEntity->pev->globalname) );
		...
		// my modifications to the follow target, to make it offset rather than exact on
		// this fuxes up on linux with an number of players
/*		if (pEntity && FBitSet (pEntity->pev->spawnflags, SF_FOLLOW_TARGET ) )
		{
			// find the models target
			edict_t    *pentTarget = NULL;
			pentTarget = FIND_ENTITY_BY_TARGETNAME( pentTarget, STRING(pEntity->pev->target)  );
			if(FNullEnt(pentTarget))
					return 0;

			// now save the offsets
			pEntity->pev->vuser1 = UTIL_DistanceBetween(pEntity->pev->origin, VARS( pentTarget )->origin);
			pEntity->pev->vuser2 = UTIL_DistanceBetween(pEntity->pev->angles, VARS( pentTarget )->angles);
		} */
	}
...
void DispatchThink( edict_t *pent )
{
	...
	if (pEntity)
	{
		...
		// move with a target.  relies on the think func being called often
		// take from http://articles.thewavelength.net/212/ with modifications at bp's request
		// this fuxes up on linux with an number of players
/*		if (FBitSet (pEntity->pev->spawnflags, SF_FOLLOW_TARGET ) )
		{
			// find the models target
			edict_t    *pentTarget = NULL;
			pentTarget = FIND_ENTITY_BY_TARGETNAME( pentTarget, STRING(pEntity->pev->target)  );
			...
			UTIL_SetOrigin( pEntity->pev, VARS( pentTarget )->origin + pEntity->pev->vuser1 );
			pEntity->pev->angles = VARS(pentTarget)->angles + pEntity->pev->vuser2;
		} */ 
	}
}
